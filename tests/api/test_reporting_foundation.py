...
def test_today_uses_farm_operational_date():
    ...
    period = _resolved_period(
        request,
        operational_today=date(2026, 9, 15),
    )
...
def test_whole_farm_requires_all_governed_domain_permissions():
    definition = next(
        report
        for report in REPORTS
        if report.id == "whole-farm-snapshot"
    )
...
def test_reporting_router_is_registered_once():
    ...
    assert (
        source.count(
            "from dairyos.api.reporting "
            "import router as reporting_router"
        )
        == 1
    )
...
def test_preview_foundation_does_not_claim_dataset_completion():
    source = Path(
        "src/dairyos/api/reporting.py"
    ).read_text(encoding="utf-8-sig")
...
def _animal(
    ...
    *,
    ...
    return SimpleNamespace(
        id=None,
        animal_id=animal_id,
        legacy_animal_id=None,
        ear_tag=f"EAR-{animal_id}",
        rfid=None,
        breed="TEST",
        sex=sex,
        ...
        status=status,
        is_currently_milking=is_currently_milking,
...
def _container(animals):
    ...
    return (
        SimpleNamespace(animal_repository=repository),
        repository,
    )
...
def test_animal_register_uses_all_six_canonical_categories(monkeypatch):
    monkeypatch.setattr(
        "dairyos.api.reporting.OperationalDateAuthority",
        _FixedOperationalDateAuthority,
    )

    container, repository = _container(
        _all_category_animals()
    )
    ...
    assert result["authority_status"] == "CURRENT_AUTHORITY_AVAILABLE"
...
def test_animal_population_uses_plural_herd_labels(monkeypatch):
    ...
    assert result["rows"] == [
        {
            "category": "Milking",
            "herd_total_label": "Milking Cows",
            "count": 1,
        },
        {
            "category": "Dry",
            "herd_total_label": "Dry Cows",
            "count": 1,
        },
        {
            "category": "Heifer",
            "herd_total_label": "Heifers",
            "count": 1,
        },
        {
            "category": "Female Calf",
            "herd_total_label": "Female Calves",
            "count": 1,
        },
        {
            "category": "Male Calf",
            "herd_total_label": "Male Calves",
            "count": 1,
        },
        {
            "category": "Bull",
            "herd_total_label": "Bulls",
            "count": 1,
        },
    ]
...
def test_terminal_animals_are_excluded_from_current_herd(
    monkeypatch,
    terminal_status,
):
    monkeypatch.setattr(
    ...
    assert result["record_count"] == 6
    assert not any(
        row["animal_id"] == f"EXIT-{terminal_status}"
        for row in result["rows"]
    )
...
def test_animal_population_empty_state_keeps_six_zero_categories(
    monkeypatch,
):
    monkeypatch.setattr(
        "dairyos.api.reporting.OperationalDateAuthority",
...
def test_preview_generated_at_uses_timezone_aware_farm_authority(
    monkeypatch,
):
    monkeypatch.setattr(
        "dairyos.api.reporting.OperationalDateAuthority",
...
def test_non_animal_report_remains_explicitly_unimplemented(
    monkeypatch,
):
    monkeypatch.setattr(
        "dairyos.api.reporting.OperationalDateAuthority",
    ...
    assert result["dataset_status"] == "DATASET_NOT_IMPLEMENTED"
    assert result["record_count"] == 0
    assert result["rows"] == []
    assert "No farm values have been" in result["warnings"][0]
