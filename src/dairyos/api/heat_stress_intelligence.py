...
def _thi(
    ...
    return (
        ...
        - (
            (0.55 - 0.0055 * humidity_pct)
            * (1.8 * temperature_c - 26.8)
        )
    )
...
def _action(risk: str) -> str:
    return {
        ...
        "ALERT": (
            "Increase water access checks, shade checks "
            "and observation frequency."
        ),
        "HIGH": (
            "Activate heat-abatement measures and increase "
            "animal monitoring frequency."
        ),
        "SEVERE": (
            "Immediate heat-abatement response; prioritize "
            "vulnerable and high-producing animals."
        ),
    }[risk]
...
def record_observation(
    observation: HeatStressIntelligenceObservation,
):
    if (
        observation.temperature_c < -20
        or observation.temperature_c > 60
    ):
        raise HTTPException(
            status_code=422,
            detail="Temperature outside operational range",
        )
...
def heat_stress_intelligence(
    ...
    try:
        ...
        if not observations:
            return {
                "data_status": (
                    "NO_ENVIRONMENTAL_OBSERVATION"
                ),
                "farm_id": farm_id,
                "period_days": days,
                "period_start_date": cutoff_date.isoformat(),
                "period_end_date": operational_date.isoformat(),
                "observation_count": 0,
                "latest": None,
                "summary": None,
                "actions": [],
        ...
        if consecutive_elevated >= 3:
            actions.append(
                "Sustained heat exposure detected across "
                "the latest three observations; verify "
        ...
        return {
            ...
            "summary": {
                ...
                "risk_counts": counts,
                "consecutive_elevated_observations": (
                    consecutive_elevated
                ),
                "current_risk": latest_risk,
                "alert": latest_risk in {
                    "ALERT",
                    "HIGH",
                    "SEVERE",
                },
            ...
            "definitions": {
                "thi": (
                    "Temperature-Humidity Index using "
                    "the standard dairy THI equation."
                ),
                "risk_bands": (
                    "NORMAL <68; ALERT 68-71.99; "
                    "HIGH 72-79.99; SEVERE >=80."
                ),
                "data_status": (
                    "Only persisted environmental observations "
                    "in the requested calendar-date period are used."
                ),
            },
        }
