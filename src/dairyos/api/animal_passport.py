...
def get_lifetime_passport(
    ...
    """Return the animal identity plus persisted cross-domain lifetime history."""
    animal = container.animal_repository.get_by_animal_id(animal_id)
    if animal is None:
        raise HTTPException(status_code=404, detail="Animal not found")

    milk = _for_animal(container.milk_repository.get_all(), animal_id)
    health = _for_animal(container.health_repository.get_all(), animal_id)
    breeding = _for_animal(container.breeding_repository.get_all(), animal_id)
    treatments = container.treatment_repository.get_by_animal(animal_id)
