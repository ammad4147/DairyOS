...
class MedicineWithdrawalService:
    ...
    def record_treatment(
        ...
        """Record a treatment event and return the expected clearance time."""
        ...
        clearance_time = treatment_time + timedelta(hours=required_hours)
    ...
    def check_withdrawal_status(
        ...
        return {
            ...
            "message": (
                f"Milk NOT safe. Wait {round(hours_remaining, 1)} more hours."
            ),
        }
        ...
        for treatment in treatments:
            ...
            if isinstance(treatment_time, str):
                treatment_time = datetime.fromisoformat(
                    treatment_time.replace("Z", "+00:00")
                )

            status = self.check_withdrawal_status(
                treatment["animal_id"],
                treatment["medicine_code"],
                treatment_time,
                current_time,
            )
