...
CONDITIONAL_SESSIONS: frozenset[str] = frozenset({MilkingSession.AFTERNOON.value})


class SequenceViolation(Exception):
    """Raised when a session is invalid or earlier sessions remain unsettled."""

    def __init__(self, *, operational_date, attempted_session: str, outstanding: list[str], reason: str = "OUT_OF_SEQUENCE"):
        ...
        self.reason = reason
        super().__init__(f"{self.attempted_session} cannot be recorded for {self.operational_date}.")
    ...
    def as_operator_guidance(self) -> dict:
        if self.reason == "UNSCHEDULED_SESSION":
            return {
                ...
                "message": (
                    f"{self.attempted_session} is not an expected milking session for this animal on "
                ...
                "resolutions": [{
                    ...
                    "description": "Use a session defined by the animal's effective Animal Passport schedule.",
                    "endpoint": "GET /farm/animals/{animal_id}/passport",
                }],
            }
        ...
        return {
            ...
            "message": (
                f"{self.attempted_session} cannot be recorded yet. The {blocking} session for "
                f"{_isoformat(self.operational_date)} has not been recorded or declared."
            ),
            "operational_date": _isoformat(self.operational_date),
            "attempted_session": self.attempted_session,
            ...
            "resolutions": [
                {
                    ...
                    "description": f"Enter the {blocking} milk figures.",
                    "endpoint": "POST /farm/milk",
                    "payload": {"milking_session": blocking, "production_date": _isoformat(self.operational_date)},
                },
                {
                    "action": "DECLARE_NOT_MILKED",
                    "description": f"Declare that the {blocking} milking did not happen, with a reason.",
                    "endpoint": "POST /farm/milk/not-milked",
                    "payload": {"milking_session": blocking, "operational_date": _isoformat(self.operational_date), "reason": "<governed reason>"},
                },
            ],
        }
...
class MilkSessionSequenceService:
    ...
    def __init__(self, ledger_repository, *, schedule_service=None):
        self.ledger = ledger_repository
        self.schedule_service = schedule_service

    def observed_sessions(self) -> list[str]:
        return [session for session in SESSION_ORDER if session not in CONDITIONAL_SESSIONS or self.ledger.has_session_ever(session)]

    def outstanding_before(self, operational_date, milking_session: str, *, animal=None) -> list[str]:
        ...
        if self.schedule_service is not None and animal is not None:
            ...
            if expected:
                ...
                return [session for session in expected[:position] if session not in settled]
        if not self._is_sequenced(operational_date, milking_session):
            return []
        position = SESSION_ORDER.index(milking_session)
        return [session for session in self.observed_sessions() if SESSION_ORDER.index(session) < position and session not in settled]
    ...
    def session_state(self, operational_date) -> dict:
        ...
        return {
            ...
            "settled_sessions": [
                {"milking_session": str(record.milking_session), "status": str(record.status), "reason": record.reason, "session_record_id": record.session_record_id}
                for record in records
            ],
        }

    def assert_can_record(self, operational_date, milking_session: str, *, animal=None) -> None:
        operational_date = _as_date(operational_date)
        milking_session = str(milking_session).upper()
        if self.schedule_service is not None and animal is not None:
            expected = tuple(self.schedule_service.get_expected_sessions(animal, operational_date))
            if expected and milking_session not in expected:
        ...
        outstanding = self.outstanding_before(operational_date, milking_session, animal=animal)
        if outstanding:
            raise SequenceViolation(
                operational_date=operational_date,
                attempted_session=milking_session,
    ...
    def _is_sequenced(self, operational_date, milking_session: str) -> bool:
        ...
        return earliest is None or operational_date >= earliest
