...
class DatabaseOperationalStateRepository:
    """
    SQLAlchemy persistence adapter for current operational state.

    FarmOperationalState owns the canonical operational-state fields.

    The operational_state JSON payload is also a shared persistence envelope
    used by other operational projection namespaces. Save operations therefore
    replace only fields owned by FarmOperationalState and preserve foreign
    namespaces.
    """
