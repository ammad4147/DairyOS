...
def resolve_period_range(
    ...
    today = anchor_date or OperationalDateAuthority().current_date()
...
class MilkProductionTrendIntelligenceService:
    def __init__(
        ...
        self._compatibility_dependency = (
            None if self._repository_factory is not None else repository_factory
        )
    ...
    def _record_session_values(record):
        ...
        declared_session = str(getattr(record, "milking_session", "") or "").upper()
    ...
    def _daily_animal_snapshot(self, records, animal, histories, target_date):
        ...
        frequency = self._schedule_service.get_frequency_for_date(
            animal, target_date, history=histories
        )
        ...
        for record in records:
            if str(getattr(record, "animal_id", "")) != animal_id:
                continue
            timestamp = _record_date(record, "production_date", "recorded_at")
        ...
        aggregate = {
            ...
            "session_ledger": True,
            "status": "RECORDED",
            "morning_yield": sum(entered["MORNING"]) if "MORNING" in entered and entered["MORNING"] else None,
            "afternoon_yield": sum(entered["AFTERNOON"]) if "AFTERNOON" in entered and entered["AFTERNOON"] else None,
            "evening_yield": sum(entered["EVENING"]) if "EVENING" in entered and entered["EVENING"] else None,
            "total_yield": None,
        }
        semantics = evaluate_sessions(aggregate, frequency)
        if semantics["missing_sessions"]:
            return {
                ...
                "frequency": frequency,
                "complete": False,
                "missing_sessions": list(semantics["missing_sessions"]),
                "total_litres": None,
            }
        aggregate["total_yield"] = daily_total(aggregate)
        return {
            "date": target_date.isoformat(),
            "animal_id": animal_id,
            "frequency": frequency,
            "complete": True,
            "missing_sessions": [],
            "total_litres": round(aggregate["total_yield"], 2),
        }
    ...
    def _animal_histories(self, factory, animals):
        ...
        for animal in animals:
            animal_id = str(getattr(animal, "animal_id", ""))
            try:
                result[animal_id] = repo.get_milking_frequency_history(animal_id)
            except AttributeError:
                result[animal_id] = []
        return result

    @staticmethod
    def _governed_milking_animals(animals, histories_by_animal, schedule_service, operational_date):
        governed = []
        for animal in animals:
            if not bool(getattr(animal, "active", True)):
                continue
            animal_id = str(getattr(animal, "animal_id", ""))
            history = list(histories_by_animal.get(animal_id, []))
            if not history and not bool(getattr(animal, "is_currently_milking", False)):
                continue
            frequency = schedule_service.get_frequency_for_date(
                animal, operational_date, history=history
            )
            if frequency in AnimalMilkingScheduleService.FREQUENCY_MAP:
                governed.append(animal)
        return governed

    def _complete_daily_totals(self, records, animals, histories_by_animal, start_date, end_date):
        daily_totals = {}
        current = start_date
        schedule_service = self._schedule_service
        while current <= end_date:
            governed_animals = self._governed_milking_animals(
                animals, histories_by_animal, schedule_service, current
            )
            complete_animals = []
            for animal in governed_animals:
                animal_id = str(getattr(animal, "animal_id", ""))
                snapshot = self._daily_animal_snapshot(
                    records, animal, histories_by_animal.get(animal_id, []), current
                )
                if snapshot and snapshot["complete"]:
                    complete_animals.append(snapshot)
            if governed_animals and len(complete_animals) == len(governed_animals):
                daily_totals[current] = round(sum(item["total_litres"] for item in complete_animals), 2)
    ...
    def _eligible_animals(factory):
        return [
            animal for animal in factory.animal().get_all()
            if bool(getattr(animal, "active", True))
        ]

    def generate(self, operational_state: Any = None, as_of_date: Optional[date] = None, period_days: int = 7, **kwargs: Any) -> TrendIntelligenceResult:
        target_date = as_of_date or OperationalDateAuthority().current_date()
        if isinstance(target_date, datetime):
            target_date = target_date.date()
        period = PERIOD_BY_DAYS.get(period_days)
        if period is None:
            raise ValueError(f"unsupported period_days: {period_days}")
        factory = self._get_factory()
        owns_factory = self._repository_factory is None
        try:
            records = factory.milk().get_all()
            animals = self._eligible_animals(factory)
            histories = self._animal_histories(factory, animals)
            start = target_date - timedelta(days=period_days - 1)
            daily_totals = self._complete_daily_totals(records, animals, histories, start, target_date)
            current_total = daily_totals.get(target_date)
            prior_dates = [day for day in daily_totals if day < target_date]
            prior_date = max(prior_dates) if prior_dates else None
            prior_total = daily_totals.get(prior_date) if prior_date else None
            comparison_status = "COMPARED" if current_total is not None and prior_total is not None else "NO_COMPARISON"
            variance_litres = None
            variance_percentage = None
            if comparison_status == "COMPARED":
                variance_litres = round(current_total - prior_total, 2)
                if prior_total:
                    variance_percentage = round((variance_litres / prior_total) * 100, 1)
            series = [
                {"date": day.isoformat(), "total_yield": round(total, 2)}
                for day, total in sorted(daily_totals.items())
            ]
            total_series = round(sum(item["total_yield"] for item in series), 2)
            direction = "UNKNOWN"
            if len(series) >= 2:
                first, last = series[0]["total_yield"], series[-1]["total_yield"]
            ...
            trend = {
                ...
                "series": series,
                "data_status": "LIVE_PERSISTED_DATA" if series else "NO_DATA",
            ...
            return TrendIntelligenceResult({
                ...
                "daily_total": round(current_total, 2) if current_total is not None else None,
                "total_litres": round(current_total, 2) if current_total is not None else None,
                "total_yield": round(current_total, 2) if current_total is not None else None,
    ...
    def get_trend_analysis(self, period: str = "7d", start_date: Optional[date] = None, end_date: Optional[date] = None, anchor_date: Optional[date] = None, factory: Optional["RepositoryFactory"] = None) -> Dict[str, Any]:
        calc_start, calc_end = resolve_period_range(period, start_date, end_date, anchor_date)
        ...
        try:
            ...
            animals = self._eligible_animals(working_factory)
            histories = self._animal_histories(working_factory, animals)
            daily_totals = self._complete_daily_totals(records, animals, histories, calc_start, calc_end)
            series = [
                {"date": day.isoformat(), "total_yield": round(total, 2)}
                for day, total in sorted(daily_totals.items())
            ]
            total = round(sum(item["total_yield"] for item in series), 2)
            trend = {
                "period": period,
            ...
            return {
                **trend,
                "period_days": (calc_end - calc_start).days + 1,
                "trend": trend,
            }
        finally:
            if owns_factory:
