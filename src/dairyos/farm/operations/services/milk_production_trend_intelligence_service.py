...
class MilkProductionTrendIntelligenceService:
    def __init__(
        ...
        self._repository_factory = (
            repository_factory
            if self._looks_like_repository_factory(
                repository_factory
            )
            else None
        )
    ...
    def _daily_animal_snapshot(
        ...
        frequency = (
            self._schedule_service.get_frequency_for_date(
                animal,
                target_date,
                history=histories,
            )
        ...
        expected = (
            self._schedule_service.get_expected_sessions(
                animal,
                target_date,
                history=histories,
            )
        ...
        for record in records:
            ...
            for session, value in (
                self._record_session_values(record).items()
            ):
    ...
    def _animal_histories(
        ...
        for animal in animals:
            ...
            try:
                result[animal_id] = (
                    repo.get_milking_frequency_history(
                        animal_id
                    )
                )
    ...
    def _governed_milking_animals(
        ...
        for animal in animals:
            ...
            # milking frequency history alone MUST NOT classify
            # an animal as Milking. The animal must currently be
            # in the milking state.
            if not bool(
                getattr(
                    animal,
            ...
            if (
                frequency
                in AnimalMilkingScheduleService.FREQUENCY_MAP
            ):
    ...
    def _complete_daily_totals(
        ...
        while current <= end_date:
            governed_animals = (
                self._governed_milking_animals(
                    animals,
                    histories_by_animal,
                    self._schedule_service,
                    current,
    ...
    def _recorded_daily_totals(
        ...
        """
        Return actual persisted milk observations by date.

        Unlike _complete_daily_totals(), this intentionally includes
        partial current-day production. A live operational trend must
        show milk that has already been recorded.
        """
        daily_totals = {}
        ...
        while current <= end_date:
            ...
            if governed_ids:
                ...
                for record in records:
                    ...
                    if animal_id not in governed_ids:
                        continue
                    ...
                    values = self._record_session_values(
                        record
                    )
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        owns_factory = (
            self._repository_factory is None
        )
        ...
        try:
            ...
            recorded_daily_totals = (
                self._recorded_daily_totals(
                    records,
                    animals,
                    histories,
                    start,
            ...
            prior_complete_dates = [
                day
                for day in complete_daily_totals
                if day < target_date
            ]

            prior_date = (
                max(prior_complete_dates)
                if prior_complete_dates
                else None
            )

            prior_total = (
                complete_daily_totals.get(
                    prior_date
                )
                if prior_date
                else None
