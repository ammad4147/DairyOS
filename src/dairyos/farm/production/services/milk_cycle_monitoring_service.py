...
class MilkCycleMonitoringService:
        ...
        try:
            ...
            if not expected:
                self._finding(
                    ...
                    detail=f"{animal_id} received a milk entry on {production_date.isoformat()} but has no governed two- or three-session milking frequency.",
                    subject_id=str(animal_id),
                    dedupe_key=f"MILK_FREQUENCY_MISSING:{animal_id}",
                )
                return {"status": "NO_GOVERNED_FREQUENCY", "frequency": frequency, "operational_date": production_date.isoformat()}

            rows = rf.milk().get_by_animal_id(str(animal_id))
            current = next((row for row in rows if _as_date(row.production_date) == production_date and bool(row.session_ledger)), None)
            if current is None:
                return {"status": "NO_LEDGER_ROW", "frequency": frequency, "operational_date": production_date.isoformat()}

            current_session = str(milking_session).upper()
            current_payload = {
                "animal_id": str(animal_id), "production_date": production_date,
                "session_ledger": True, "status": current.status,
            ...
            if current_session not in expected:
                self._finding(
                    rf, severity="HIGH", title=f"Unscheduled milking session for {animal_id}",
                    detail=f"{current_session} was entered on {production_date.isoformat()} but the animal was governed at {frequency} on that operational date.",
                    subject_id=str(animal_id),
                    dedupe_key=f"MILK_UNSCHEDULED_SESSION:{animal_id}:{production_date.isoformat()}:{current_session}",
                )
                return {"status": "UNSCHEDULED_SESSION", "frequency": frequency, "operational_date": production_date.isoformat()}

            index = SESSION_ORDER[current_session]
            earlier_expected = [session for session in expected if SESSION_ORDER[session] < index]
            earlier_missing = [session for session in earlier_expected if session in missing_sessions(current_payload, frequency)]

            if earlier_missing:
                self._finding(
                    rf, severity="HIGH", title=f"Missed milk yield interval for {animal_id}",
                    detail=f"A {current_session} yield was entered for {production_date.isoformat()} while earlier scheduled interval(s) {', '.join(earlier_missing)} remain unentered.",
                    subject_id=str(animal_id),
                    dedupe_key=f"MILK_MISSED_INTERVAL:{animal_id}:{production_date.isoformat()}",
                )

            records = [{
                "animal_id": str(row.animal_id), "production_date": row.production_date,
                "session_ledger": bool(row.session_ledger), "status": row.status,
                "morning_yield": row.morning_yield, "afternoon_yield": row.afternoon_yield,
                "evening_yield": row.evening_yield, "total_yield": row.total_yield,
            } for row in rows]
            ...
            if comparison and comparison.get("severity"):
                self._finding(
                    rf, severity=comparison["severity"],
                    title=f"{animal_id} milk yield declined on {production_date.isoformat()}",
                    detail=f"{comparison['previous_date']}: {comparison['previous']:.1f} L -> {comparison['current_date']}: {comparison['current']:.1f} L ({abs(comparison['percent']):.1f}% decline).",
                    subject_id=str(animal_id), dedupe_key=f"MILK_DAILY_DROP:{animal_id}",
                )

            return {
                "status": comparison.get("status") if comparison else "NO_COMPARISON",
                "comparison": comparison, "missing_intervals": earlier_missing,
                "frequency": frequency, "expected_sessions": list(expected),
                "operational_date": production_date.isoformat(),
            }
        finally:
            if owns_factory:
    ...
    @staticmethod
    def _finding(rf, *, severity: str, title: str, detail: str, dedupe_key: str, subject_id: str | None = None):
        return MilkFindingService(rf.operational_findings()).raise_or_update(
            severity=severity, title=title, detail=detail,
            subject_type="ANIMAL" if subject_id else "FARM",
            subject_id=subject_id if subject_id else "MILK",
