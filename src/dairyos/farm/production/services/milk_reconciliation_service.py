...
VALID_DISPOSITIONS = frozenset({"SOLD", "CALF_FEED", "DOMESTIC_USE", "WASTAGE", "OTHER", "WITHDRAWAL"})
...
class MilkReconciliationService:
    ...
    def _production_total(cls, production_date: date, production_repository=None) -> dict:
        ...
        if not complete:
            return {"date": production_date.isoformat(), "complete": False, "daily_total": None, "total_litres": None, "saleable_litres": None, "withdrawal_litres": None}
        ...
        return {"date": production_date.isoformat(), "complete": True, "daily_total": total, "total_litres": total, "saleable_litres": saleable, "withdrawal_litres": withdrawal}
    ...
    def _active_disposition_sum(dispositions, disposition_type_filter=None):
        return sum(
            ...
            and (disposition_type_filter is None or str(item.disposition_type).upper() == disposition_type_filter)
        )

    @classmethod
    def validate_disposition_quantity(cls, *, production_basis: dict, dispositions, disposition_type: str, quantity_litres: float, exclude_id: int | None = None) -> None:
        ...
        active = [item for item in dispositions if str(getattr(item, "status", "RECORDED") or "RECORDED").upper() != "VOID" and (exclude_id is None or getattr(item, "id", None) != exclude_id)]
        if disposition_type == "WITHDRAWAL":
            available = max(float(production_basis.get("withdrawal_litres") or 0.0) - cls._active_disposition_sum(active, "WITHDRAWAL"), 0.0)
            if float(quantity_litres) > available + 0.01:
                raise ValueError("WITHDRAWAL milk disposition exceeds recorded withdrawal litres: " f"available {available:.3f} L, requested {float(quantity_litres):.3f} L.")
            return
    ...
    def reconcile(self, production_date: date, *, raise_finding: bool = True):
        ...
        try:
            ...
            current = self._production_total(production_date, production_repository=production_repository)
            dispositions = repo.get_by_date(production_date)

            if not current.get("complete", False):
                return {"production_date": production_date.isoformat(), "production_complete": False, "produced_litres": None, "biological_production_litres": None, "withdrawal_litres": None, "accounted_litres": round(sum(float(item.quantity_litres) for item in dispositions), 3), "sold_litres": round(sum(float(item.quantity_litres) for item in dispositions if str(item.disposition_type).upper() == "SOLD"), 3), "non_sale_accounted_litres": round(sum(float(item.quantity_litres) for item in dispositions if str(item.disposition_type).upper() != "SOLD"), 3), "unaccounted_litres": None, "over_accounted_litres": None, "sale_value": round(sum(float(item.amount_due or 0.0) for item in dispositions if str(item.disposition_type).upper() == "SOLD"), 2), "cash_received": round(sum(float(item.amount_received or 0.0) for item in dispositions if str(item.disposition_type).upper() == "SOLD"), 2), "receivable_outstanding": round(sum(float(item.receivable_outstanding) for item in dispositions if str(item.disposition_type).upper() == "SOLD"), 2), "status": "PRODUCTION_INCOMPLETE", "dispositions": [self._serialize_disposition(item) for item in dispositions]}
            ...
            active = [item for item in dispositions if str(getattr(item, "status", "RECORDED") or "RECORDED").upper() != "VOID"]
            withdrawal_accounted = sum(float(item.quantity_litres) for item in active if str(item.disposition_type).upper() == "WITHDRAWAL")
            ordinary_accounted = sum(float(item.quantity_litres) for item in active if str(item.disposition_type).upper() != "WITHDRAWAL")
            accounted = ordinary_accounted + withdrawal_accounted
            sold = sum(float(item.quantity_litres) for item in active if str(item.disposition_type).upper() == "SOLD")
            non_sale = sum(float(item.quantity_litres) for item in active if str(item.disposition_type).upper() not in {"SOLD", "WITHDRAWAL"})
            sale_value = sum(float(item.amount_due or 0.0) for item in active if str(item.disposition_type).upper() == "SOLD")
            cash_received = sum(float(item.amount_received or 0.0) for item in active if str(item.disposition_type).upper() == "SOLD")
    ...
    def _serialize_disposition(item):
        return {"id": item.id, "disposition_type": item.disposition_type, "quantity_litres": item.quantity_litres, "sale_id": item.sale_id, "counterparty": item.counterparty, "selling_price_per_litre": item.selling_price_per_litre, "amount_due": item.amount_due, "amount_received": item.amount_received, "receivable_outstanding": item.receivable_outstanding, "notes": item.notes, "recorded_by": item.recorded_by}

    def record_disposition(self, *, production_date: date, disposition_type: str, quantity_litres: float, sale_id: str | None = None, counterparty: str | None = None, selling_price_per_litre: float | None = None, notes: str | None = None, recorded_by: str | None = None):
        disposition_type = str(disposition_type).strip().upper()
