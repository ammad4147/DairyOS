...
import COPOfficializationPanel from './components/COPOfficializationPanel';
   ...
   return (
     <div className="app-shell" style={{display:'flex',flexDirection:'column',height:'100vh',minWidth:0,background:'#0b0f19',color:'#f8fafc',overflow:'hidden',fontFamily:'sans-serif'}}>
      <header style={{height:60,background:'#0f172a',borderBottom:'1px solid #1e293b',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 12px',zIndex:50,flexShrink:0,boxShadow:'0 4px 6px -1px rgba(0,0,0,.3)',minWidth:0}}>
       ...
       <nav style={{display:'flex',gap:3,justifyContent:'center',flex:1,minWidth:0,margin:'0 4px',overflowX:'hidden',overflowY:'hidden'}}>{navItems.map(tab=>{const isActive=currentView===tab.id;const colorMap:Record<string,string>={dashboard:'#2563eb',animals:'#d97706',milk:'#0284c7',feed:'#059669',finance:'#15803d',breeding:'#ea580c',health:'#dc2626',vaccination:'#15803d',cop:'#7e22ce'};const themeColor=colorMap[tab.id]||'#475569';return <button key={tab.id} onClick={()=>setCurrentView(tab.id)} style={{display:'flex',alignItems:'center',gap:3,flex:'0 0 auto',background:isActive?themeColor:'#1e293b',color:'#f8fafc',border:isActive?`1px solid ${themeColor}`:'1px solid #475569',padding:'5px',borderRadius:6,cursor:'pointer',fontSize:9,fontWeight:isActive?'800':'700',whiteSpace:'nowrap',transition:'all 0.2s',boxShadow:isActive?'0 0 0 1px rgba(255,255,255,.12) inset':'none'}}><span style={{color:isActive?'#fff':'#e2e8f0',display:'flex',alignItems:'center'}}>{tab.icon}</span>{tab.label}</button>})}</nav>
      ...
      <main style={{flex:1,minHeight:0,minWidth:0,overflowY:'auto',overflowX:'hidden',background:'#0b0f19',position:'relative'}}>
       ...
       {currentView==='milk'&&<MilkTab initialOpenModal={autoOpenYieldModal} onModalClose={()=>setAutoOpenYieldModal(false)} herdMasterList={herdMasterList} onSaveYield={()=>setDashboardRefreshVersion(prev=>prev+1)} realTimeTodaySold={todayMilkSoldLiters} onOpenAnimalPassport={openLinkedPassport} onOperationalChanged={async()=>{setDashboardRefreshVersion(prev=>prev+1);await refreshAlerts()}}/>}
