      ...
      <header style={{ height: '60px', background: '#0f172a', borderBottom: '1px solid #1e293b', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 12px', zIndex: 50, flexShrink: 0, boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.3)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <h1 style={{ margin: 0, fontSize: '13px', fontWeight: 'bold', color: '#f8fafc', letterSpacing: '0.2px', whiteSpace: 'nowrap' }}>
              {farmName}
            </h1>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: isActive ? '0 0 10px rgba(14, 165, 233, 0.5)' : 'none',
                  whiteSpace: 'nowrap'
                }}
              >
                {tab.icon} {tab.label}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              <Bell size={14} />
              {activeCount > 0 && (
                <span style={{ position: 'absolute', top: '-4px', right: '-4px', minWidth: '16px', height: '16px', background: '#ef4444', border: '2px solid #0f172a', borderRadius: '50%', color: '#fff', fontSize: '9px', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 2px' }}>
                  {activeCount}
                </span>
              )}
            </button>
            ...
            {showNotifications && (
              <div style={{ position: 'absolute', right: 0, top: '40px', width: '380px', background: '#111827', border: '1px solid #1f2937', borderRadius: '8px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.75)', padding: '12px', zIndex: 100 }}>
                <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#fff', borderBottom: '1px solid #1f2937', paddingBottom: '8px', marginBottom: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={() => { setCurrentView('audit'); setShowNotifications(false); }} style={{ background: 'none', border: 'none', color: '#38bdf8', fontSize: '11px', cursor: 'pointer', textDecoration: 'underline' }}>
                  ...
                  {alerts.filter(a => a.status !== 'RESOLVED').map(n => {
                    ...
                    return (
                      <div
                        ...
                        style={{ fontSize: '12px', background: isReinstated ? 'rgba(239, 68, 68, 0.3)' : '#161f30', padding: '10px', borderRadius: '6px', borderLeft: `4px solid ${isReinstated ? '#dc2626' : (n.currentLevel === 'RED' ? '#ef4444' : '#f59e0b')}`, border: isReinstated ? '1px solid #ef4444' : 'none', cursor: 'pointer' }}
                      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#1e293b', border: '1px solid #334155', padding: '4px 10px', borderRadius: '20px' }}>
            ...
            <button onClick={handleLogout} title="Log Out" style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', padding: '0 0 0 2px', display: 'flex', alignItems: 'center' }}>
              <LogOut size={14} />
            </button>
          </div>
        </div>
