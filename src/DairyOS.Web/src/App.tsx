...
import { LayoutDashboard, Calculator, BarChart3, DollarSign, Milk, HeartPulse, Activity, Settings, Plus, Bell, Clock, LogOut, Wheat, Users } from 'lucide-react';
import './App.css';
...
export default function MainAppShell(){
 ...
 const [currentTime,setCurrentTime]=useState(new Date());useEffect(()=>{const timer=setInterval(()=>setCurrentTime(new Date()),1000);return()=>clearInterval(timer)},[]);
 const {alerts,activeCount}=useAlertAudit();const [showNotifications,setShowNotifications]=useState(false);
 const handleOpenYieldEntry=()=>{setAutoOpenYieldModal(true);setCurrentView('milk')};const handleLogout=()=>{localStorage.removeItem('dairyos_token');localStorage.removeItem('dairyos_user');setCurrentUser(null)};
 const [animals,setAnimals]=useState<BackendAnimal[]>([]);
 const refreshAnimals=useCallback(async()=>{try{const response=await fetch(`${API_BASE}/farm/animals?active_only=false`);if(!response.ok)throw new Error(`Unable to load herd (${response.status})`);const records=await response.json() as BackendAnimal[];setAnimals(records)}catch(error){console.error('DairyOS herd register load failed:',error)}},[]);
 useEffect(()=>{if(currentUser)void refreshAnimals()},[currentUser,refreshAnimals]);
 const herdMasterList=animals.map(toUiAnimal);
 const [showAnimalModal,setShowAnimalModal]=useState(false),[todayYield,setTodayYield]=useState(133),[todayMilkSoldLiters,setTodayMilkSoldLiters]=useState(110),[accountsReceivable,setAccountsReceivable]=useState(23400);
 ...
 return <div className="app-shell" style={{display:'flex',flexDirection:'column',height:'100vh',minWidth:0,background:'#0b0f19',color:'#f8fafc',overflow:'hidden',fontFamily:'sans-serif'}}>
  <header style={{height:60,background:'#0f172a',borderBottom:'1px solid #1e293b',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 12px',zIndex:50,flexShrink:0,boxShadow:'0 4px 6px -1px rgba(0,0,0,.3)',minWidth:0}}>
   ...
   <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}><div style={{display:'flex',alignItems:'center',gap:6,background:'#1e293b',border:'1px solid #334155',padding:'4px 8px',borderRadius:16,fontSize:10,color:'#cbd5e1'}}><Clock size={12} color="#38bdf8"/><span style={{fontWeight:'bold',color:'#fff'}}>{currentTime.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'})}</span></div>
    <div style={{position:'relative'}}><button onClick={()=>setShowNotifications(!showNotifications)} style={{position:'relative',background:'#1e293b',border:'1px solid #334155',padding:6,borderRadius:'50%',color:'#f59e0b',cursor:'pointer',display:'flex'}}><Bell size={14}/>{activeCount>0&&<span style={{position:'absolute',top:-4,right:-4,minWidth:16,height:16,background:'#ef4444',border:'2px solid #0f172a',borderRadius:'50%',color:'#fff',fontSize:9,fontWeight:'bold',display:'flex',alignItems:'center',justifyContent:'center'}}>{activeCount}</span>}</button>{showNotifications&&<div style={{position:'absolute',right:0,top:40,width:380,maxWidth:'min(380px,calc(100vw - 20px))',background:'#111827',border:'1px solid #1f2937',borderRadius:8,boxShadow:'0 20px 25px -5px rgba(0,0,0,.75)',padding:12,zIndex:100}}><div style={{fontSize:12,fontWeight:'bold',borderBottom:'1px solid #1f2937',paddingBottom:8,marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}><span>Active Warnings ({activeCount})</span><button onClick={()=>{setCurrentView('audit');setShowNotifications(false)}} style={{background:'none',border:'none',color:'#38bdf8',fontSize:11,cursor:'pointer',textDecoration:'underline'}}>Open Full Audit Register</button></div><div style={{display:'flex',flexDirection:'column',gap:8,maxHeight:340,overflowY:'auto'}}>{alerts.filter(a=>a.status!=='RESOLVED').map(n=>{const isReinstated=n.status==='REINSTATED';return <div key={n.id} onClick={()=>{setCurrentView('audit');setShowNotifications(false)}} style={{fontSize:11,background:isReinstated?'rgba(239,68,68,.3)':'#161f30',padding:9,borderRadius:6,borderLeft:`4px solid ${isReinstated?'#dc2626':n.currentLevel==='RED'?'#ef4444':'#f59e0b'}`,cursor:'pointer'}}><div style={{color:isReinstated?'#fee2e2':'#e2e8f0',fontWeight:'bold'}}>{isReinstated&&'🚨 '}{n.title}</div><div style={{fontSize:10,color:isReinstated?'#fca5a5':'#94a3b8',marginTop:4}}>{n.details}</div></div>})}</div></div>}</div>
    <button onClick={()=>setCurrentView('settings')} style={{background:currentView==='settings'?'#0ea5e9':'#1e293b',border:currentView==='settings'?'1px solid #7dd3fc':'1px solid #334155',padding:6,borderRadius:'50%',color:'#e2e8f0',cursor:'pointer',display:'flex'}}><Settings size={14}/></button><div style={{display:'flex',alignItems:'center',gap:8,background:'#1e293b',border:'1px solid #334155',padding:'4px 8px',borderRadius:20}}><div title={`${currentUser.fullName} (${currentUser.role})`} style={{width:24,height:24,borderRadius:'50%',background:'#38bdf8',color:'#0f172a',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:'bold',fontSize:11}}>{currentUser.fullName.split(' ').map(n=>n[0]).slice(0,2).join('')}</div><button onClick={handleLogout} title="Log Out" style={{background:'none',border:'none',color:'#94a3b8',cursor:'pointer',display:'flex'}}><LogOut size={14}/></button></div>
   </div>
  </header>
  <main style={{flex:1,minHeight:0,minWidth:0,overflowY:'auto',overflowX:'hidden',background:'#0b0f19',position:'relative'}}>
   {currentView==='dashboard'&&<UnifiedDashboard onNavigate={v=>setCurrentView(v)} onOpenYieldModal={handleOpenYieldEntry} onOpenPassport={id=>setSelectedPassportAnimalId(id)} herdMasterList={herdMasterList} realTimeTodayYield={todayYield} realTimeReceivables={accountsReceivable}/>} 
   ...
   {currentView==='finance'&&<FinanceTab onSaveSale={liters=>setTodayMilkSoldLiters(prev=>prev+liters)} onUpdateReceivables={setAccountsReceivable}/>} 
   {currentView==='feed'&&<FeedTab/>}
   {currentView==='coml'&&<COML/>}{currentView==='analytics'&&<Analytics/>}{currentView==='audit'&&<AuditTab/>}{currentView==='settings'&&<SettingsTab onFarmProfileUpdate={p=>{setFarmName(p.farmName);setFarmLocation(p.location)}}/>}
   {currentView==='milk'&&<MilkTab initialOpenModal={autoOpenYieldModal} onModalClose={()=>setAutoOpenYieldModal(false)} herdMasterList={herdMasterList} onSaveYield={added=>setTodayYield(prev=>prev+added)} realTimeTodaySold={todayMilkSoldLiters}/>} 
   {currentView==='health'&&<HealthTab onOpenPassport={id=>setSelectedPassportAnimalId(id)} herdMasterList={herdMasterList}/>} {currentView==='breeding'&&<BreedingTab onOpenPassport={id=>setSelectedPassportAnimalId(id)} herdMasterList={herdMasterList}/>} 
  </main>
  {selectedPassportAnimalId&&<AnimalPassportModal animalId={selectedPassportAnimalId} onClose={()=>setSelectedPassportAnimalId(null)} onSave={handleRegisterAnimal}/>} {showAnimalModal&&<AnimalPassportModal animalId="NEW-ANIMAL" onClose={()=>setShowAnimalModal(false)} onSave={handleRegisterAnimal}/>} 
 </div>
}
