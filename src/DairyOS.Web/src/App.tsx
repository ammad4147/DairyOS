...
export default function MainAppShell() {
  ...
  const [animals, setAnimals] = useState<HerdAnimal[]>([
    ...
    { id: 'TD-003', breed: 'Cholistani', category: 'Milking Cows', age: '3 Years', status: 'Healthy', frequency: 'TWICE_DAILY', earTag: 'PK-LHR-003', gender: 'Female' },
    { id: 'TD-004', breed: 'Nili-Ravi (Buffalo)', category: 'Dry Cows', age: '6 Years', status: 'Under Treatment', frequency: 'NONE', earTag: 'PK-LHR-004', gender: 'Female' },
    { id: 'TD-005', breed: 'Holstein Cross', category: 'Heifers', age: '18 Months', status: 'Growing', frequency: 'NONE', earTag: 'PK-LHR-005', gender: 'Female' },
    { id: 'TD-006', breed: 'Sahiwal', category: 'Female Calves', age: '3 Months', status: 'Weaned', frequency: 'NONE', earTag: 'PK-LHR-006', gender: 'Female' },
    { id: 'TD-007', breed: 'Holstein', category: 'Male Calves', age: '2 Months', status: 'Fattening', frequency: 'NONE', earTag: 'PK-LHR-007', gender: 'Male' },
    { id: 'TD-008', breed: 'Sahiwal Sire', category: 'Bulls', age: '4 Years', status: 'Active Breeding', frequency: 'NONE', earTag: 'PK-LHR-008', gender: 'Male' },
    { id: 'TD-009', breed: 'Holstein Purebred', category: 'Milking Cows', age: '4.5 Years', status: 'Healthy', frequency: 'THRICE_DAILY', earTag: 'PK-LHR-009', gender: 'Female' },
    { id: 'TD-014', breed: 'Jersey Cross', category: 'Milking Cows', age: '3.5 Years', status: 'Healthy', frequency: 'THRICE_DAILY', earTag: 'PK-LHR-014', gender: 'Female' },
  ]);
      ...
      <div className="main-viewport" style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, boxSizing: 'border-box' }}>
        <header style={{ height: '54px', background: '#111827', borderBottom: '1px solid #1f2937', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', boxSizing: 'border-box', position: 'relative', zIndex: 50 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    {alerts.filter(a => a.status !== 'RESOLVED').map(n => {
                      ...
                      return (
                        <div
                          ...
                          style={{ fontSize: '11px', background: isReinstated ? 'rgba(239, 68, 68, 0.3)' : '#161f30', padding: '8px 10px', borderRadius: '6px', borderLeft: `3px solid ${isReinstated ? '#dc2626' : (n.currentLevel === 'RED' ? '#ef4444' : '#f59e0b')}`, border: isReinstated ? '1px solid #ef4444' : 'none', cursor: 'pointer' }}
                        >
            ...
            <button onClick={() => setCurrentView('settings')} style={{ background: currentView === 'settings' ? '#38bdf8' : '#1e293b', border: '1px solid #334155', padding: '6px', borderRadius: '50%', color: currentView === 'settings' ? '#0f172a' : '#94a3b8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Settings">
              <Settings size={15} />
            </button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px', marginBottom: '14px' }}>
                ...
                <div style={{ background: '#111827', border: '1px solid #1f2937', borderLeft: '4px solid #38bdf8', padding: '8px 10px', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#38bdf8' }}>
                    {animals.filter(a => a.status === 'Milking' || a.category === 'Milking' || a.category === 'Milking Cows' || (a.gender === 'Female' && a.stage === 'Lactating')).length}
                  </div>
                </div>
                <div style={{ background: '#111827', border: '1px solid #1f2937', borderLeft: '4px solid #94a3b8', padding: '8px 10px', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#94a3b8' }}>
                    {animals.filter(a => a.status === 'Dry' || a.category === 'Dry' || a.category === 'Dry Cows' || a.stage === 'Dry').length}
                  </div>
                </div>
                <div style={{ background: '#111827', border: '1px solid #1f2937', borderLeft: '4px solid #f59e0b', padding: '8px 10px', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#f59e0b' }}>
                    {animals.filter(a => a.category === 'Heifer' || a.category === 'Heifers' || a.stage === 'Heifer').length}
                  </div>
                </div>
                <div style={{ background: '#111827', border: '1px solid #1f2937', borderLeft: '4px solid #ec4899', padding: '8px 10px', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#ec4899' }}>
                    {animals.filter(a => (a.gender === 'Female' && (a.category?.includes('Calf') || a.stage?.includes('Calf'))) || a.category === 'Female Calves' || a.category === 'Female Calf').length}
                  </div>
                </div>
                <div style={{ background: '#111827', border: '1px solid #1f2937', borderLeft: '4px solid #3b82f6', padding: '8px 10px', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#3b82f6' }}>
                    {animals.filter(a => (a.gender === 'Male' && (a.category?.includes('Calf') || a.stage?.includes('Calf'))) || a.category === 'Male Calves' || a.category === 'Male Calf').length}
                  </div>
                </div>
                <div style={{ background: '#111827', border: '1px solid #1f2937', borderLeft: '4px solid #a855f7', padding: '8px 10px', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#a855f7' }}>
                    {animals.filter(a => a.category === 'Bull' || a.category === 'Bulls' || a.category === 'Sire' || (a.gender === 'Male' && !a.category?.includes('Calf') && !a.stage?.includes('Calf'))).length}
                  </div>
                </div>
              </div>
