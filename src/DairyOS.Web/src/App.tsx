...
const entryConfigs: Record<string, OperationalEntryConfig> = {
    milk: {
        endpoint: "/farm/milk",
        title: "Record Milk",
        description: "Select a milking animal, choose the current session, and enter its yield.",
        fields: [animalField, operatorField],
    },
    feed: {
        endpoint: "/farm/feed",
        ...
        fields: [
            ...
            { name: "animal_id", label: "Animal (optional)", type: "animal" }, operatorField,
        ],
    },
    health: {
...
function App() {
    ...
    useEffect(() => {
        let cancelled = false;
        const loadJson = <T,>(path: string) => fetch(apiUrl(path)).then((response) => {
            if (!response.ok) throw new Error(`${path} failed`);
            return response.json() as Promise<T>;
        });
        loadJson<{ farm_name?: string }>("/settings").then((payload) => {
            if (!cancelled && payload.farm_name) setFarmName(payload.farm_name);
        }).catch(() => {});
        loadJson<{ status?: string }>("/health").then((payload) => {
            if (!cancelled) setSystemHealth(payload.status ?? "HEALTHY");
        }).catch(() => { if (!cancelled) setSystemHealth("OFFLINE"); });
        loadJson<{ farm_status?: string }>("/operations/dashboard").then((payload) => {
            if (!cancelled) setFarmStatus(payload.farm_status ?? "UNKNOWN");
        }).catch(() => { if (!cancelled) setFarmStatus("UNAVAILABLE"); });
    ...
    useEffect(() => {
        const onKeyDown = (event: KeyboardEvent) => {
            ...
            if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") {
                ...
                setHelpOpen(true);
            } else if (!editing && event.key === "?" && !event.shiftKey) {
                setHelpOpen(true);
            }
        };
    ...
    const openDocumentation = (manualId: "operator" | "technical", sectionId: string) => {
        setHelpOpen(false);
        setHelpSelection({ manualId, sectionId });
    };
        ...
        <div className="dairyos-shell">
            <header className="dairyos-topbar">
                <div className="brand-block">
                    ...
                    <div><div className="brand-name">DairyOS</div><div className="brand-subtitle">{farmName}</div></div>
                </div>
                <div className="topbar-status">
                    <div className="status-pill"><span className={`status-dot ${systemHealth === "OFFLINE" ? "danger" : "live"}`} /><span>System {systemHealth}</span></div>
                    ...
                    <button type="button" className="help-launcher" onClick={() => setHelpOpen(true)} title="Search Help and Manuals">Help <kbd>Ctrl K</kbd></button>
                </div>
            </header>
            ...
            <div className="dairyos-body">
                <aside className={`dairyos-sidebar ${mobileNavOpen ? "open" : ""}`}>
                    ...
                    <div className="sidebar-footer">
                        ...
                        <button type="button" className="sidebar-help-button" onClick={() => setHelpOpen(true)}>Help &amp; Documentation</button>
                    </div>
                </aside>
                ...
                <main className="dairyos-main">
                    {view === "command" && <MainDashboard onNavigate={selectView} />}
                    {view === "animals" && <AnimalRegistry onNavigate={selectView} />}
                    {view === "settings" && (
                        <>
                            <div className="page-heading"><div><div className="breadcrumb">DairyOS / Settings</div><h1>{activeNavigation.label}</h1><p>{activeNavigation.description}</p></div><div className="page-meta">Local documentation is available offline.</div></div>
                            <Settings />
                            {!helpSelection && <DocumentationView compact />}
                            {helpSelection && <DocumentationView compact initialManual={helpSelection.manualId} initialSectionId={helpSelection.sectionId} onClose={() => setHelpSelection(null)} />}
                        </>
                    )}
