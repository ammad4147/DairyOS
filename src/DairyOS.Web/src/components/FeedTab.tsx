...
export default function FeedTab() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_BASE}/farm/feed-inventory/authoritative`);
      if (!response.ok) throw new Error('Authoritative feed inventory unavailable.');
      const body = await response.json();
      setFeedItems((body.items ?? []).map((row: any) => ({
        id: Number(row.id),
        item: String(row.item),
        unit: String(row.unit || 'unit'),
        purchased: Number(row.purchased_from_finance || 0),
        used: Number(row.used_from_operations || 0),
        balance: Number(row.balance || 0),
        status: row.status,
      })));
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unable to load Feed data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const recordUsage = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const catalog = await fetch(`${API_BASE}/farm/feed-inventory/dashboard`);
      if (catalog.ok) {
        const dashboard = await catalog.json();
        const exists = (dashboard.items || []).some((row: any) => row.item === current.item);
        if (!exists) {
          const create = await fetch(`${API_BASE}/farm/feed-inventory/items`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ item: current.item, category: 'FEED', unit: current.unit, reorder_level: 0, active: true }),
          });
      ...
      const response = await fetch(`${API_BASE}/farm/feed-inventory/movements`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ item: current.item, quantity, movement_type: usageType, unit: current.unit, notes: notes || null, recorded_by: 'WEB' }),
      });
      ...
      setMessage(`${usageType === 'CONSUMPTION' ? 'Consumption' : 'Wastage'} recorded for ${current.item}.`);
      setUsageQty('');
      setNotes('');
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unable to record feed usage.');
    } finally {
      setSaving(false);
    }
  };
