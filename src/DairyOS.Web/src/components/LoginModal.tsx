    ...
    try {
      ...
      if (response.ok) {
        ...
        const userObj = {
          ...
          role,
          fullName: username === 'admin' ? 'Ammad Hassan' : username,
        };
        localStorage.setItem('dairyos_token', data.access_token || 'demo-token');
        localStorage.setItem('dairyos_user', JSON.stringify(userObj));
        onLoginSuccess(userObj);
      } else {
        // Fallback for demo local testing if server responds with error
        if (username === 'admin' && password === 'dairyos') {
          const userObj = { username: 'admin', role: 'OWNER', fullName: 'Ammad Hassan' };
          localStorage.setItem('dairyos_token', 'demo-token-123');
          localStorage.setItem('dairyos_user', JSON.stringify(userObj));
          onLoginSuccess(userObj);
        } else {
      ...
      // Local fallback in offline development mode
      if (username === 'admin' && password === 'dairyos') {
        const userObj = { username: 'admin', role: 'OWNER', fullName: 'Ammad Hassan' };
        localStorage.setItem('dairyos_token', 'demo-token-123');
        localStorage.setItem('dairyos_user', JSON.stringify(userObj));
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            ...
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <User size={15} color="#64748b" style={{ position: 'absolute', left: '12px' }} />
              <input
                type="text"
                value={username}
          ...
          >
            {loading ? 'Authenticating...' : 'Sign In to Farm System'} <ArrowRight size={15} />
          </button>
        </form>

        <div style={{ marginTop: '24px', textAlign: 'center', fontSize: '10px', color: '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
          <ShieldCheck size={13} color="#34d399" />
          <span>Operational Security & Multi-Role Attribution Active</span>
        </div>
      </div>
    </div>
  );
