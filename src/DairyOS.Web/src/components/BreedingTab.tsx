...
export default function BreedingTab({ onOpenPassport, herdMasterList = [], onChanged }: Props) {
  ...
  const [semenLots, setSemenLots] = useState<SemenLot[]>([]);
  const [formSemenLotId, setFormSemenLotId] = useState('');
  ...
  const [formReturnToMilkingDate, setFormReturnToMilkingDate] = useState('');
  ...
  const loadRecords = async () => {
    ...
    setError(null);
    try {
      const [breeding, overview, cyclePayload, analyticsPayload, semenPayload, current] = await Promise.all([
        ...
        getJson<any>('/farm/breeding/semen-stock'),
      ...
      setSemenLots(Array.isArray(semenPayload?.available_lots) ? semenPayload.available_lots : []);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (eventType === 'CALVING') {
        if (!formCalfSex) throw new Error('Select whether the calf is female or male.');
        if (!formReturnToMilkingDate) throw new Error('Enter the planned date the mother will return to the milking herd.');
        if (formReturnToMilkingDate < formDate) throw new Error('Return-to-milking date cannot be before the calving date.');
        event_type = 'calving';
      }
      if (eventType === 'LOSS') {
      ...
      if (eventType === 'AI') {
        if (!formSemenLotId) throw new Error('Select an available purchased semen lot.');
      }
      await postJson('/farm/breeding', {
        ...
        result,
        semen_or_bull: eventType === 'AI' ? undefined : formSire.trim() || null,
        semen_lot_id: eventType === 'AI' ? Number(formSemenLotId) : undefined,
        notes: formNotes || undefined,
        operator: formInseminator.trim() || 'WEB',
        timestamp: formDate,
        calf_sex: eventType === 'CALVING' ? formCalfSex : undefined,
        planned_return_to_milking_date: eventType === 'CALVING' ? formReturnToMilkingDate : undefined,
      });
