  ...
  const [records, setRecords] = useState<BreedingRecord[]>([
    {
      ...
      aiDate: '2026-03-28',
      sireCode: 'ABS-SUPERIOR-991',
      semenType: 'Sexed Semen (90% Female)',
      inseminator: 'Dr. Tariq Mahmood',
      daysPregnant: 146,
      expectedCalving: '2027-01-02',
      pdDueDate: '2026-05-02 (Confirmed +)',
      notes: 'Strong fetal heartbeat detected at 60d ultrasound.'
    },
    {
      id: 'BRD-2026-02',
      tag: 'TD-002',
      status: 'Inseminated (Pending PD)',
      aiDate: '2026-08-06',
      sireCode: 'WWS-SAHIWAL-STAR',
      semenType: 'Conventional',
      inseminator: 'Dr. Tariq Mahmood',
      daysPregnant: 15,
      expectedCalving: '2027-05-13',
      pdDueDate: '2026-09-10 (Day 35)',
      notes: 'Natural standing heat observed morning milking.'
    },
    {
      id: 'BRD-2026-03',
      tag: 'TD-003',
      status: 'Standing Heat',
      aiDate: '-',
      sireCode: 'Assigned: US-HO-882',
      semenType: 'Sexed Semen (90% Female)',
      inseminator: '-',
      daysPregnant: 0,
      expectedCalving: '-',
      pdDueDate: '-',
      notes: 'Clear mucus discharge & standing heat at 06:00 AM.'
    },
    {
      id: 'BRD-2026-04',
      tag: 'TD-005',
      status: 'Open / Ready',
      aiDate: '-',
      sireCode: 'Candidate: ABS-EASY-CALV',
      semenType: 'Sexed Semen (90% Female)',
      inseminator: '-',
      daysPregnant: 0,
      expectedCalving: '-',
      pdDueDate: '-',
      notes: 'Virgin Heifer (18 months, 345 kg BW) ready for 1st service.'
    },
  ]);

  // Form States
  const [formTag, setFormTag] = useState('TD-003');
  ...
  const handleSaveEvent = (e: React.FormEvent) => {
    ...
    if (eventType === 'AI') {
      ...
      const newRecord: BreedingRecord = {
        ...
        aiDate: formDate,
        sireCode: formSire,
        semenType: formSemenType,
        inseminator: formInseminator,
        daysPregnant: 0,
    ...
    <div style={{ padding: '20px', color: '#fff', height: '100%', overflowY: 'auto', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <button
          ...
          style={{ background: '#fb923c', color: '#0f172a', border: 'none', padding: '8px 16px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px' }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '16px' }}>
        <div style={{ background: '#111827', border: '1px solid #1f2937', padding: '12px', borderRadius: '6px', borderLeft: '3px solid #fb923c' }}>
          ...
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#fb923c' }}>
            {records.filter(r => r.status === 'Standing Heat').length} Animals
          </div>
        ...
        <div style={{ background: '#111827', border: '1px solid #1f2937', padding: '12px', borderRadius: '6px', borderLeft: '3px solid #60a5fa' }}>
          ...
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#60a5fa' }}>
            {records.filter(r => r.status === 'Inseminated (Pending PD)').length} Animals
          </div>
        ...
        <div style={{ background: '#111827', border: '1px solid #1f2937', padding: '12px', borderRadius: '6px', borderLeft: '3px solid #a78bfa' }}>
          ...
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#a78bfa' }}>
            {records.filter(r => r.status === 'Confirmed Pregnant').length} Animals
          </div>
            ...
            {records.map(r => (
              <tr key={r.id} style={{ borderBottom: '1px solid #1a2234' }}>
                <td style={{ padding: '10px 12px' }}>
                  <button
                    onClick={() => openPassportHandler(r.tag)}
                    style={{ background: 'none', border: 'none', color: '#38bdf8', fontWeight: 'bold', cursor: 'pointer', padding: 0, textDecoration: 'underline', fontSize: '12px' }}
                ...
                <td style={{ padding: '10px 12px' }}>
                  <span style={{
                    ...
                    fontWeight: 'bold',
                    background:
                      r.status === 'Confirmed Pregnant' ? 'rgba(167, 139, 250, 0.2)' :
                      r.status === 'Standing Heat' ? 'rgba(251, 146, 60, 0.2)' :
                      r.status === 'Inseminated (Pending PD)' ? 'rgba(96, 165, 250, 0.2)' : 'rgba(52, 211, 153, 0.2)',
                    color:
                      r.status === 'Confirmed Pregnant' ? '#a78bfa' :
                      r.status === 'Standing Heat' ? '#fb923c' :
                      r.status === 'Inseminated (Pending PD)' ? '#60a5fa' : '#34d399'
                  }}>
                    {r.status}
                  </span>
                </td>
                ...
                <td style={{ padding: '10px 12px', color: '#cbd5e1' }}>
                  <span style={{ color: r.semenType.includes('Sexed') ? '#ec4899' : '#94a3b8', fontWeight: 'bold', fontSize: '11px' }}>
                    {r.semenType}
                  </span>
                </td>
                <td style={{ padding: '10px 12px' }}>
                  ...
                  {r.expectedCalving !== '-' && (
                    <div style={{ fontSize: '10px', color: '#34d399' }}>Calving: {r.expectedCalving} ({r.daysPregnant}d)</div>
                  )}
                </td>
          ...
          <div style={{ background: '#111827', border: '1px solid #fb923c', borderRadius: '10px', width: '480px', padding: '22px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px', marginBottom: '14px' }}>
              <button
                ...
                style={{ background: eventType === 'AI' ? '#fb923c' : '#1e293b', color: eventType === 'AI' ? '#0f172a' : '#cbd5e1', border: 'none', padding: '6px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}
              >
                Insemination (AI)
              </button>
              <button
                type="button"
                onClick={() => setEventType('HEAT')}
                style={{ background: eventType === 'HEAT' ? '#fb923c' : '#1e293b', color: eventType === 'HEAT' ? '#0f172a' : '#cbd5e1', border: 'none', padding: '6px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}
              >
                Heat Observation
              </button>
              <button
                type="button"
                onClick={() => setEventType('PD')}
                style={{ background: eventType === 'PD' ? '#fb923c' : '#1e293b', color: eventType === 'PD' ? '#0f172a' : '#cbd5e1', border: 'none', padding: '6px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}
              >
                Pregnancy Check (PD)
              </button>
            </div>
            ...
            <form onSubmit={handleSaveEvent} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div>
                ...
                <select value={formTag} onChange={e => setFormTag(e.target.value)} style={{ width: '100%', background: '#1e293b', color: '#fff', border: '1px solid #334155', padding: '8px', borderRadius: '4px', fontSize: '12px' }}>
                  ...
                  <option value="TD-005">TD-005 (Holstein Heifer)</option>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px' }}>
                <button type="button" onClick={() => setShowEventModal(false)} style={{ background: '#334155', color: '#fff', border: 'none', padding: '8px 14px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>
                  Cancel
                </button>
                <button type="submit" style={{ background: '#fb923c', color: '#0f172a', border: 'none', padding: '8px 14px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px' }}>
                  Save Breeding Entry
                </button>
              </div>
            </form>
      ...
      {activeModalPassport && (
        <AnimalPassportModal animalId={activeModalPassport} onClose={() => setActiveModalPassport(null)} />
      )}
