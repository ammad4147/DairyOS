...
function trendText(milk: DashboardMilkSummary): string {
    ...
    if (typeof milk.trend_percent === "number") {
    ...
    const yesterday = numberValue(
        milk.yesterday_litres ?? milk.previous_day_litres,
    );
...
function trendClass(milk: DashboardMilkSummary): string {
    const direction =
        String(milk.trend_direction ?? "").toLowerCase();
...
function ProductionCard({
    ...
    const yesterday =
        milk.yesterday_litres
        ?? milk.previous_day_litres;
    ...
    const maxHistory = Math.max(
        ...
        numberValue(milk.today_litres) ?? 0,
        1,
    );
    ...
    return (
        <section className="production-card">
            ...
            <div className="production-stat-grid">
                <div>
                    <span>Yesterday</span>
                    <strong>{litres(yesterday)}</strong>
                </div>

                <div>
                    <span>7-day average</span>
                    <strong>{litres(average)}</strong>
                </div>
            </div>
            ...
            {history.length > 0 ? (
                <div className="production-history">
                    ...
                    <div
                        ...
                        aria-label="Milk production trend"
                    >
                                ...
                                >
                                    <div className="trend-bar-wrap">
                                        <div
                                            ...
                                            title={`${point.date}: ${litres(point.litres)}`}
                                        />
                                    </div>
                ...
                <div className="production-history-empty">
                    ...
                    <p>
                        Historical yield will appear here as
                        animal-linked milk records accumulate.
                    </p>
                </div>
            )}
            ...
            {widgets.length > 0 && (
                <div className="production-footer">
                    <span>
                        Data linked to operational milk records.
                    </span>
                </div>
            )}
        </section>
