...
export default function Analytics() {
  ...
  const [milkDimension, setMilkDimension] = useState<'DIM' | 'THI' | 'MONTH'>('MONTH');
  ...
  const load = async () => {
    ...
    try {
      ...
      const body = await response.json() as LiveAnalytics;
      setData(body);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to load live analytics.');
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ padding: '20px', color: '#fff', height: '100%', overflowY: 'auto', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: '0 0 4px 0', fontSize: '18px', color: '#38bdf8', display: 'flex', alignItems: 'center', gap: '8px' }}><Layers size={20} /> Multivariate Analytics Query Engine</h2>
          <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8' }}>Cross-correlate persisted operational factors and biological performance.</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button style={{ background: '#111827', border: '1px solid #1f2937', color: '#64748b', padding: '6px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'not-allowed' }} title="Manual analytics event logging is not yet connected to a persisted analytics input contract"><AlertTriangle size={12} /> Log Manual Event (not connected)</button>
          <button onClick={() => { setIsCompareMode(!isCompareMode); if (isCompareMode && selectedTabs.length > 1) setSelectedTabs([selectedTabs[0]]); }} style={{ background: isCompareMode ? '#0284c7' : '#111827', color: '#fff', border: `1px solid ${isCompareMode ? '#38bdf8' : '#1f2937'}`, padding: '6px 14px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}><Sliders size={13} /> {isCompareMode ? 'Comparison Mode: ON' : 'Enable Compare Mode'}</button>
          <button onClick={() => void load()} style={{ background: '#111827', border: '1px solid #1f2937', color: '#cbd5e1', padding: '6px 9px', borderRadius: '6px', cursor: 'pointer' }} title="Refresh live analytics"><RefreshCw size={13} /></button>
        </div>
      </div>

      {error && <div style={{ marginBottom: 10, padding: 9, background: 'rgba(239,68,68,.10)', border: '1px solid #ef4444', color: '#fecaca', borderRadius: 6, fontSize: 10 }}>{error}</div>}
      {data && <div style={{ marginBottom: 10, fontSize: 9, color: '#64748b' }}>Source: {data.data_status === 'LIVE_PERSISTED_DATA' ? 'LIVE PERSISTED DATA' : 'NO DATA'} • 30-day governed analytics window</div>}

      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {tabsMeta.map(tab => {
          const isSelected = selectedTabs.includes(tab.id);
          return <button key={tab.id} onClick={() => handleTabClick(tab.id)} style={{ background: isSelected ? tab.color + '22' : '#111827', color: isSelected ? tab.color : '#94a3b8', border: `1px solid ${isSelected ? tab.color : '#1f2937'}`, padding: '8px 16px', borderRadius: '8px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s ease' }}><span style={{ color: tab.color }}>{tab.icon}</span>{tab.label}</button>;
        })}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: getGridLayout(), gap: '16px', alignItems: 'start' }}>
        {selectedTabs.includes('milk') && (
          <div style={{ background: '#0f172a', border: '1px solid #38bdf844', borderRadius: '10px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1e293b', paddingBottom: '8px' }}><div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#38bdf8', fontWeight: 'bold', fontSize: '14px' }}><Milk size={16} /> Production vs. Environment Analyzer</div></div>
            <div style={{ background: '#111827', padding: '12px', borderRadius: '8px', border: '1px solid #1f2937' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                <div><label style={{ fontSize: '9px', color: '#64748b', display: 'block', marginBottom: '2px' }}>Y-Axis (Metric)</label><select value={milkMetric} onChange={e => setMilkMetric(e.target.value as any)} style={{ width: '100%', background: '#1e293b', color: '#fff', border: '1px solid #334155', padding: '6px', borderRadius: '4px', fontSize: '11px' }}><option value="YIELD">Daily Yield (L)</option><option value="BUTTERFAT" disabled>Butterfat % (no live composition contract)</option></select></div>
                <div><label style={{ fontSize: '9px', color: '#64748b', display: 'block', marginBottom: '2px' }}>X-Axis (Dimension)</label><select value={milkDimension} onChange={e => setMilkDimension(e.target.value as any)} style={{ width: '100%', background: '#1e293b', color: '#fff', border: '1px solid #334155', padding: '6px', borderRadius: '4px', fontSize: '11px' }}><option value="MONTH">Calendar Date</option><option value="THI">Heat Stress (THI)</option><option value="DIM">Observation Sequence</option></select></div>
                <div><label style={{ fontSize: '9px', color: '#64748b', display: 'block', marginBottom: '2px' }}>Filter (Cohort)</label><select value={milkCohort} onChange={e => setMilkCohort(e.target.value as any)} style={{ width: '100%', background: '#1e293b', color: '#fff', border: '1px solid #334155', padding: '6px', borderRadius: '4px', fontSize: '11px' }}><option value="ALL">All governed animals</option><option value="HF" disabled>Holstein only (not exposed by live contract)</option><option value="CROSS" disabled>Crossbred only (not exposed by live contract)</option></select></div>
              </div>
            </div>
            <div style={{ height: '220px', background: '#111827', padding: '12px', borderRadius: '8px', border: '1px solid #1f2937' }}>
              {loading ? <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b', fontSize: 11 }}>Loading live milk analytics…</div> : milkData.length === 0 ? <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b', fontSize: 11 }}>No complete milk/environment observations are available for this period.</div> : <ResponsiveContainer width="100%" height="100%"><ComposedChart data={milkData} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} /><XAxis dataKey={milkDimension === 'THI' ? 'thi' : (milkDimension === 'DIM' ? 'dim' : 'period')} stroke="#64748b" tick={{ fontSize: 9 }} /><YAxis allowDecimals={false} yAxisId="left" stroke="#64748b" tick={{ fontSize: 9 }} domain={['auto', 'auto']} /><Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', fontSize: '11px' }} /><Legend wrapperStyle={{ fontSize: '10px', paddingTop: '10px' }} />{milkMetric === 'YIELD' && <Line yAxisId="left" type="monotone" dataKey="yield_total" name="Farm Daily Yield (L)" stroke="#38bdf8" strokeWidth={3} dot={{ r: 4 }} />}{milkMetric === 'BUTTERFAT' && <Area yAxisId="left" type="monotone" dataKey="yield_total" name="Unavailable live composition" fill="rgba(251, 191, 36, 0.2)" stroke="#fbbf24" strokeWidth={2} />}</ComposedChart></ResponsiveContainer>}
            </div>
            <div style={{ background: 'rgba(56, 189, 248, 0.1)', border: '1px solid rgba(56, 189, 248, 0.3)', padding: '10px', borderRadius: '6px', display: 'flex', gap: '8px' }}><ThermometerSun size={18} color="#38bdf8" style={{ flexShrink: 0 }} /><div style={{ fontSize: '11px', color: '#cbd5e1' }}><strong style={{ color: '#38bdf8' }}>Engine Insight:</strong> {hasThiSeries ? 'Live complete milk days are joined only where persisted THI observations exist. No unsupported seasonal benchmark is inferred.' : 'No persisted THI observations overlap complete milk days in this period; no heat-stress correlation is asserted.'}</div></div>
          </div>
        )}
        ...
        {selectedTabs.includes('financial') && <div style={{ background: '#0f172a', border: '1px solid #fbbf2444', borderRadius: '10px', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '300px' }}><div style={{ textAlign: 'center', color: '#94a3b8', fontSize: '12px' }}><DollarSign size={32} color="#fbbf24" style={{ margin: '0 auto 8px' }} />{comlPerL != null ? <>Feed Cost/L: PKR {Number(feedPerL || 0).toFixed(2)}<br/>OPEX/L: PKR {Number(opexPerL || 0).toFixed(2)}<br/>CMPL/L: PKR {Number(comlPerL).toFixed(2)}</> : 'No persisted financial + milk evidence is available for live unit economics.'}</div></div>}

        {selectedTabs.includes('dynamics') && <div style={{ background: '#0f172a', border: '1px solid #22d3ee44', borderRadius: '10px', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '300px' }}><div style={{ textAlign: 'center', color: '#94a3b8', fontSize: '12px' }}><Database size={32} color="#22d3ee" style={{ margin: '0 auto 8px' }} />Active persisted herd: {currentHerd}<br/>{Object.entries(data?.herd_dynamics.lifecycle_counts || {}).map(([key, value]) => <React.Fragment key={key}>{key}: {value}<br/></React.Fragment>)}</div></div>}
      </div>
    </div>
  );
}