...
function operationalMetric(animal: Animal): string {
    ...
    return dim === null || dim === undefined ? "—" : `${String(dim)} d`;
}
        ...
        <section className="animal-registry">
            ...
            <div className="animal-table-wrap">
                <table className="animal-table">
                    <thead><tr><th>Tag / Ear ID</th><th>Name / Alias</th><th>Category / Group</th><th>Current Status</th><th>Age / DOB</th><th>DIM</th></tr></thead>
                    <tbody>
                        {filteredAnimals.map((animal) => (
                            <tr key={String(animal.id)} tabIndex={0} onClick={() => void openPassport(animal)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openPassport(animal); } }}>
            ...
            {showEntry && <div className="animal-modal-backdrop" role="presentation" onClick={() => setShowEntry(false)}><div className="animal-modal" role="dialog" aria-modal="true" aria-labelledby="animal-entry-title" onClick={(event) => event.stopPropagation()}><header className="animal-modal-header"><div><h3 id="animal-entry-title">Register Animal</h3><p>Register through the existing governed animal workflow.</p></div><button type="button" className="animal-close" onClick={() => setShowEntry(false)} aria-label="Close">×</button></header><form onSubmit={submitAnimal}><div className="animal-form-grid">
                {([["animal_type","Animal Type","text"],["ear_tag","Ear Tag","text"],["rfid","RFID","text"],["breed","Breed","text"],["sex","Sex","text"],["date_of_birth","Date of Birth","date"],["dam_id","Dam ID","text"],["sire_id","Sire ID","text"],["production_group","Production Group","text"],["location","Location","text"]] as const).map(([name,label,type]) => <label key={name}>{label}<input type={type} value={form[name]} onChange={(event) => updateForm(name, event.target.value)} /></label>)}
                ...
                {form.operational_mode === "MILKING" ? <label>Milking Frequency<select value={form.milking_frequency} onChange={(event) => updateForm("milking_frequency", event.target.value)}><option value="TWICE_DAILY">Twice daily</option><option value="THRICE_DAILY">Three times daily</option></select></label> : <><label>Non-milking Category<select value={form.non_milking_category} onChange={(event) => updateForm("non_milking_category", event.target.value)}><option value="HEALTH">Health restriction</option><option value="DRY_REPRODUCTIVE">Dry / reproductive break</option><option value="MILK_SEPARATELY">Milk separately</option><option value="PERMANENT">Permanent non-milking</option><option value="OTHER">Other operational</option></select></label><label>Documented Reason<input required value={form.non_milking_reason} onChange={(event) => updateForm("non_milking_reason", event.target.value)} /></label></>}
                </div><div className="animal-form-footer"><button type="button" className="animal-button secondary" onClick={() => setShowEntry(false)}>Cancel</button><button type="submit" className="animal-button primary" disabled={saving}>{saving ? "Saving…" : "Register Animal"}</button></div></form></div></div>}

            {selected && <div className="animal-drawer-backdrop" role="presentation" onClick={() => { setSelected(null); setPassport(null); }}><aside className="animal-passport-drawer" role="dialog" aria-modal="true" aria-labelledby="animal-passport-title" onClick={(event) => event.stopPropagation()}><header className="animal-modal-header"><div><div className="animal-registry-kicker">ANIMAL PASSPORT</div><h3 id="animal-passport-title">{selected.ear_tag ?? selected.animal_id}</h3><p>{nameOrAlias(selected)} · {currentStatus(selected)}</p></div><button type="button" className="animal-close" onClick={() => { setSelected(null); setPassport(null); }} aria-label="Close passport">×</button></header>
                {passportLoading ? <div className="animal-passport-loading">Loading complete passport…</div> : passport ? <div className="animal-passport"><div className="animal-passport-identity">{Object.entries(passport.animal ?? selected).slice(0, 12).map(([key,value]) => <div key={key}><span>{humanize(key)}</span><strong>{display(value)}</strong></div>)}</div><div className="animal-passport-section"><h4>Schedule</h4><div className="animal-passport-row"><span>Frequency</span><strong>{frequencyLabel(passport.schedule?.effective?.milking_frequency ?? selected.milking_frequency)}</strong></div><div className="animal-passport-row"><span>Expected Sessions</span><strong>{display(passport.schedule?.effective?.expected_sessions?.join(", "))}</strong></div></div>{Object.entries(passport.history ?? {}).map(([domain, records]) => Array.isArray(records) && records.length > 0 ? <div className="animal-passport-section" key={domain}><h4>{domainLabel(domain)} · {records.length}</h4>{records.slice(0, 10).map((record, index) => <div className="animal-passport-row" key={`${domain}-${index}`}><span>{recordTitle(record, domain)}</span><strong>{display(Object.values(record).slice(0, 3).map(display).join(" · "))}</strong></div>)}</div> : null)}{passport.timeline && passport.timeline.length > 0 && <div className="animal-passport-section"><h4>Timeline · {passport.timeline.length}</h4>{passport.timeline.slice(0, 20).map((item,index)=><div className="animal-passport-row" key={`${item.domain}-${item.timestamp}-${index}`}><span>{domainLabel(item.domain)} · {formatDate(item.timestamp)}</span><strong>{recordTitle(item.record,item.domain)}</strong></div>)}</div>}</div> : <div className="animal-passport-loading">Unable to load passport details.</div>}
            </aside></div>}
        </section>
    );
}
