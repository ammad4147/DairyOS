    ...
    >
      ...
      {showPeriodLog && (
        <div style={{ marginTop: 10, padding: 10, background: '#0f172a', border: '1px solid #1f2937', borderRadius: 7, overflowX: 'auto' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', marginTop: 8, fontSize: 9 }}>
            <thead><tr style={{ color: '#94a3b8', borderBottom: '1px solid #1f2937' }}><th style={{ padding: '6px 8px', textAlign: 'left', width: '16%' }}>Date</th><th style={{ padding: '6px 8px', textAlign: 'right', width: '13%' }}>Milk (L)</th><th style={{ padding: '6px 8px', textAlign: 'right', width: '17%' }}>Feed Cost/L</th><th style={{ padding: '6px 8px', textAlign: 'right', width: '19%' }}>Estimated OPEX/L</th><th style={{ padding: '6px 8px', textAlign: 'right', width: '19%' }}>Estimated COP/L</th><th style={{ padding: '6px 8px', textAlign: 'left', width: '16%' }}>Status</th></tr></thead>
            <tbody>{periodLog.length ? periodLog.map(row => <tr key={row.date} style={{ borderTop: '1px solid #1a2234' }}><td style={{ padding: '6px 8px', textAlign: 'left', fontVariantNumeric: 'tabular-nums' }}>{row.date}</td><td style={{ padding: '6px 8px', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{Number(row.milk_litres || 0).toFixed(2)}</td><td style={{ padding: '6px 8px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{money(row.feed_cost_per_liter)}</td><td style={{ padding: '6px 8px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{money(row.estimated_opex_per_liter)}</td><td style={{ padding: '6px 8px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{money(row.estimated_cop_per_liter)}</td><td style={{ padding: '6px 8px', textAlign: 'left' }}>{row.status}</td></tr>) : <tr><td colSpan={6} style={{ padding: 12, textAlign: 'center', color: '#94a3b8' }}>No daily COP records yet.</td></tr>}</tbody>
          </table>
        </div>
      )}
