...
export default function COPOfficializationPanel() {
  ...
  const [autoData, setAutoData] =
    useState<AutoData | null>(null);

  const [official, setOfficial] =
    useState<OfficialPayload | null>(null);
  ...
  const load = async (
    ...
    try {
      ...
      const [
        ...
        fetch(
          `${API_BASE}/farm/coml/integrated?${integratedQuery}`,
        ),
        fetch(
      ...
      if (!autoResponse.ok) {
        throw new Error(
          autoBody?.detail
          || 'Auto COP calculation is unavailable.',
        );
      }
  ...
  const feedTotal = Number(
    autoData?.costs?.feed_total ?? 0,
  );

  const opexTotal = Number(
    autoData?.costs?.opex_total ?? 0,
  );
  ...
  const officialRecord =
    official?.record ?? null;

  const officialMonth =
    String(
      officialRecord?.month_start || '',
    ).slice(0, 7);

  const selectedMonthIsOfficial =
    officialMonth === month;
  ...
  const makeOfficial = async () => {
    ...
    try {
        ...
        `${API_BASE}/farm/coml/lock`,
        {
          ...
          body: JSON.stringify({
            ...
            opex_cost_per_liter:
              Number(opexPerL),
            notes:
              `AUTO-CALCULATED COP made official for `
      ...
      setMessage(
        `Auto COP for ${month} has been adopted as the official COP record.`,
      );
    ...
    >
      ...
      >
        <div>
          ...
          >
            Auto COP uses governed
            TMR whole-herd Feed Cost / L,
            authoritative milk production,
            and Finance OPEX / L.
            Review it before making the
            calculated value official.
          </div>
        </div>
        ...
        <>
          ...
          >
            ...
            <Metric
              ...
              detail={
                `${money(feedTotal)} whole-herd TMR cost`
              }
            />
          ...
          >
            ...
            Governed TMR ration ×
            active DairyOS herd.
            Bulk Finance Feed purchases
            supply ingredient quantity
            and price authority but are
          ...
          >
            ...
            >
              {selectedMonthIsOfficial
                ? (
                  `Official COP already exists for ${month}: `
                  + `${money(
                    officialRecord
