...
import { Building, Mail, Save, ShieldCheck, Sliders, UserPlus, UserX, UserCheck, KeyRound } from 'lucide-react';
...
export default function SettingsTab({ onFarmProfileUpdate }: SettingsTabProps) {
  const [activeTab,setActiveTab]=useState<'FARM'|'STANDARDS'|'USERS'|'EMAIL'>('FARM');
  ...
  const [newUsername,setNewUsername]=useState(''),[newPassword,setNewPassword]=useState(''),[newRole,setNewRole]=useState('MANAGER'),[newTitle,setNewTitle]=useState(''),[newEmail,setNewEmail]=useState('');
  ...
  const createUser=async(e:React.FormEvent)=>{e.preventDefault();setError('');setMessage('');try{
    ...
    setNewUsername('');setNewPassword('');setNewTitle('');setNewEmail('');setPermissions([]);setMessage(`User ${d.username} created with ${pd.permissions.length} explicit permissions.`);await loadUsers();
  ...
  return <div style={{padding:24,color:'#fff',height:'100%',overflowY:'auto'}}>
    ...
    {activeTab==='USERS'&&canUsers&&<div style={{display:'grid',gridTemplateColumns:'minmax(260px,360px) minmax(0,1fr)',gap:14,alignItems:'start'}}>
      <div style={{display:'grid',gap:14}}>
        ...
        <form onSubmit={createUser} style={card}>
          ...
          <input placeholder="Job title (optional)" value={newTitle} onChange={e=>setNewTitle(e.target.value)} style={field}/>
          <input type="email" placeholder="Personal email (optional)" value={newEmail} onChange={e=>setNewEmail(e.target.value)} style={field}/>
          <select value={newRole} onChange={e=>{setNewRole(e.target.value);applyPreset(e.target.value)}} style={field}>{Object.keys(matrix?.roles||{OWNER:{},MANAGER:{},MILKER:{}}).map(role=><option key={role}>{role}</option>)}</select>
          <div style={muted}>Preset is only a starting point. The owner can customize every permission below.</div>
    ...
    {activeTab==='EMAIL'&&canEmail&&<div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(320px,420px)',gap:14,alignItems:'start'}}>
      <section style={card}><h3 style={title}>DairyOS Sender</h3><div style={muted}>All nightly digests and DairyOS-generated email are sent using this identity. Database settings override deployment defaults.</div>
        ...
        <label style={label}>Display name</label><input value={emailConfig.sender_display_name??''} onChange={e=>setEmailConfig({...emailConfig,sender_display_name:e.target.value})} style={field}/>
        <label style={label}>SMTP host</label><input value={emailConfig.smtp_host??''} onChange={e=>setEmailConfig({...emailConfig,smtp_host:e.target.value})} style={field}/>
        <div style={{display:'grid',gridTemplateColumns:'120px 1fr',gap:8}}><div><label style={label}>Port</label><input type="number" value={emailConfig.smtp_port??587} onChange={e=>setEmailConfig({...emailConfig,smtp_port:Number(e.target.value)})} style={field}/></div><div><label style={label}>SMTP username</label><input value={emailConfig.smtp_username??''} onChange={e=>setEmailConfig({...emailConfig,smtp_username:e.target.value})} style={field}/></div></div>
        <label style={label}>SMTP password {emailConfig.password_configured&&'(stored)'}</label><input type="password" value={emailPassword} onChange={e=>setEmailPassword(e.target.value)} placeholder={emailConfig.password_configured?'Leave blank to keep current password':''} style={field}/>
        <label style={{display:'flex',gap:7,alignItems:'center',fontSize:10,color:'#cbd5e1',marginBottom:12}}><input type="checkbox" checked={emailConfig.use_tls!==false} onChange={e=>setEmailConfig({...emailConfig,use_tls:e.target.checked})}/>Use TLS</label>
        <button onClick={saveEmail} style={button}><Mail size={14}/>Save Email Settings</button>
      </section>
