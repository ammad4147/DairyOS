...
function renderLine(line: string, index: number) {
    ...
    if (/^\d+\. /.test(line)) {
        return <div className="doc-list-item ordered" key={`${index}-${line}`}>{line}</div>;
    }
    if (line.startsWith("- ")) {
        return <div className="doc-list-item" key={`${index}-${line}`}>{line.slice(2)}</div>;
    }
...
function DocumentReader({ manual, section }: { manual: ReturnType<typeof manualById>; section: ManualSection }) {
    return (
        <article className="documentation-reader">
            <div className="documentation-reader-meta">
            ...
            <div className="documentation-body">
                {sectionLines(section.content).map(renderLine)}
            </div>
...
export function DocumentationView({
    ...
    const section = useMemo(
        () => manual.sections.find((item) => item.id === sectionId) ?? manual.sections[0],
        [manual, sectionId],
    );
    const results = useMemo(() => searchManuals(query), [query]);

    useEffect(() => {
        if (!manual.sections.some((item) => item.id === sectionId)) {
            setSectionId(manual.sections[0]?.id);
        }
    }, [manual, sectionId]);
    ...
    return (
        <div className={compact ? "documentation documentation-compact" : "documentation"}>
            <div className="documentation-header">
                <div>
                    <div className="documentation-kicker">HELP &amp; DOCUMENTATION</div>
                    <h1>{manual.title}</h1>
                    <p>{manual.purpose}</p>
                </div>
            ...
            <div className="documentation-search-row">
                <input
                    ...
                    placeholder="Search all manual topics, headings and keywords…"
                    aria-label="Search DairyOS documentation"
                />
            ...
            {query && (
                <div className="documentation-results">
                    {results.map((result) => (
                        <button
                            key={`${result.manualId}-${result.sectionId}`}
                            type="button"
                            className="documentation-result"
                            onClick={() => {
                    ...
                    <div className="documentation-tabs">
                        {MANUALS.map((item) => (
                            <button
                                key={item.id}
                                type="button"
                                className={manualId === item.id ? "selected" : ""}
                                onClick={() => {
                                    setManualId(item.id);
                    ...
                    <div className="documentation-toc">
                        {manual.sections.map((item) => (
                            <button
                                key={item.id}
                                type="button"
                                className={section.id === item.id ? "selected" : ""}
...
export function DocumentationSearchOverlay({ onClose, onOpenDocumentation }: SearchOverlayProps) {
    const [query, setQuery] = useState("");
    const results = useMemo(() => searchManuals(query), [query]);
        ...
        <div className="documentation-overlay" role="dialog" aria-modal="true" aria-label="DairyOS Help search">
            <div className="documentation-search-dialog">
                <div className="documentation-search-dialog-header">
                    <div>
                        <div className="documentation-kicker">DAIRYOS HELP</div>
                        <h2>Search Operator &amp; Technical Manuals</h2>
                    </div>
                ...
                <input
                    ...
                    value={query}
                    onChange={(event) => setQuery(event.target.value)}
                    placeholder="Try: milk reconciliation, PostgreSQL, reset protection, breeding…"
                    aria-label="Global manual search"
                />
                <div className="documentation-search-results">
                    {query.trim() ? results.map((result) => (
                        <button
                            ...
                            type="button"
                            className="documentation-result"
                            onClick={() => onOpenDocumentation(result.manualId, result.sectionId)}
                        >
                            <strong>{result.sectionTitle}</strong>
                            <span>{result.manualTitle}</span>
                            <small>{result.summary}</small>
