  ...
  return (
    <div style={{ padding: '20px', color: '#fff', height: '100%', overflowY: 'auto', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h2 style={{ margin: '0 0 4px 0', fontSize: '18px', color: '#ef4444', display: 'flex', alignItems: 'center', gap: '8px' }}><HeartPulse size={20} /> Veterinary Health & Medical Ledger</h2>
          <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8' }}>Track treatments, vaccinations, and manage milk withdrawal periods for food safety compliance.</p>
        </div>
      ...
      <div style={{ background: '#111827', border: '1px solid #1f2937', borderRadius: '8px', overflow: 'hidden' }}>
        <table style={{ width: '100%', fontSize: '13px', borderCollapse: 'collapse' }}><thead><tr style={{ background: '#161f30', color: '#94a3b8', borderBottom: '1px solid #1f2937', textAlign: 'left' }}><th style={{ padding: '12px 16px' }}>Date</th><th style={{ padding: '12px 16px' }}>Animal ID</th><th style={{ padding: '12px 16px' }}>Type & Condition</th><th style={{ padding: '12px 16px' }}>Treatment / Medication</th><th style={{ padding: '12px 16px' }}>Withdrawal</th><th style={{ padding: '12px 16px', textAlign: 'right' }}>Status</th></tr></thead><tbody>
          {loading ? null : records.map(r => <tr key={r.id} style={{ borderBottom: '1px solid #1a2234' }}><td style={{ padding: '12px 16px', color: '#94a3b8' }}>{r.date}</td><td style={{ padding: '12px 16px' }}><button onClick={() => openPassportHandler(r.tag)} style={{ background: 'none', border: 'none', color: '#38bdf8', fontWeight: 'bold', cursor: 'pointer', padding: 0, textDecoration: 'underline', fontSize: '13px' }}>#{r.tag}</button></td><td style={{ padding: '12px 16px' }}><div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>{r.type === 'Vaccination' ? <ShieldCheck size={14} color="#34d399" /> : r.type === 'Treatment' ? <Syringe size={14} color="#ef4444" /> : <FileText size={14} color="#38bdf8" />}<span style={{ color: '#e2e8f0', fontWeight: 'bold' }}>{r.condition}</span></div></td><td style={{ padding: '12px 16px', color: '#cbd5e1' }}>{r.medication}</td><td style={{ padding: '12px 16px' }}>{r.withdrawalDays > 0 ? <span style={{ background: 'rgba(245, 158, 11, 0.2)', color: '#fcd34d', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', display: 'inline-flex', alignItems: 'center', gap: '4px' }}><AlertTriangle size={12} /> {r.withdrawalDays} Days</span> : <span style={{ color: '#64748b', fontSize: '11px' }}>Clear</span>}</td><td style={{ padding: '12px 16px', textAlign: 'right' }}><span style={{ background: r.status === 'Active' ? 'rgba(239, 68, 68, 0.2)' : r.status === 'Resolved' ? 'rgba(56, 189, 248, 0.2)' : 'rgba(52, 211, 153, 0.2)', color: r.status === 'Active' ? '#fca5a5' : r.status === 'Resolved' ? '#7dd3fc' : '#6ee7b7', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold' }}>{r.status}</span></td></tr>)}
        </tbody></table>
      </div>
