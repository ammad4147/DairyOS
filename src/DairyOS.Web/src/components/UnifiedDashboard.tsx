...
export default function UnifiedDashboard({ onNavigate, onOpenYieldModal, onOpenPassport, herdMasterList = [], realTimeTodayYield, realTimeReceivables = 0 }: Props) {
  ...
  return (
    <div className="cmd-dash-wrapper" style={{ height:'calc(100vh - 75px)', overflowY:'hidden', overflowX:'hidden', display:'flex', flexDirection:'column', boxSizing:'border-box', padding:10 }}>
      ...
      <div className="cmd-content-grid" style={{ display:'grid', gridTemplateColumns:'1.2fr 0.8fr', gap:10, flex:1, minHeight:0, minWidth:0 }}>
        <div className="cmd-col" style={{ display:'flex', flexDirection:'column', gap:10, minHeight:0 }}>
          <div className="cmd-card" style={{ flex:'1.6', display:'flex', flexDirection:'column', background:'#111827', border:'1px solid #1f2937', borderRadius:8, padding:10, minHeight:0 }}>
            ...
            <div className="stat-row" style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:6, marginBottom:8, minWidth:0 }}>
              ...
              <div onClick={() => onNavigate?.('coml')} style={{ background:'#1e293b', padding:6, borderRadius:6, borderLeft:'2px solid #34d399', cursor:'pointer', minWidth:0 }} title="Cost of Production per Liter"><div style={{ fontSize:8, color:'#34d399', fontWeight:800 }}>Cost of Production/Liter {officialComl?.has_official && officialComl.month_label ? `(${officialComl.month_label})` : ''}</div><div style={{ fontSize:13, fontWeight:900, color:'#34d399', marginTop:2 }}>{officialComl?.has_official ? `PKR ${officialComl.record!.total_coml_per_liter.toFixed(2)} / L` : ''}</div></div>
            </div>
        ...
        <div className="cmd-col" style={{display:'flex',flexDirection:'column',gap:10,minHeight:0}}>
          ...
          <div className="cmd-card" style={{flex:0.85,...cardBase}}><div style={{display:'flex',alignItems:'center',gap:6,color:'#ef4444',fontWeight:800,fontSize:12,marginBottom:6,cursor:'pointer'}} onClick={()=>onNavigate?.('health')}><HeartPulse size={15}/> Health & Treatments</div><div style={{display:'flex',flexDirection:'column',gap:6,justifyContent:'center',flex:1}}><div style={healthBox}><span style={{fontWeight:800,color:'#fca5a5',fontSize:10}}><b style={{background:'#ef4444',color:'#fff',padding:'2px 4px',borderRadius:4,fontSize:8,marginRight:5}}>SICK</b>{healthData.sick} ANIMALS</span><span style={{fontSize:9,color:'#f87171'}}>Mastitis: {healthData.mastitis} | Temp: {healthData.highTemp}</span></div><div style={{...healthBox,background:'rgba(245,158,11,.10)',borderColor:'rgba(245,158,11,.3)'}}><span style={{fontWeight:800,color:'#fcd34d',fontSize:10}}>VACCINATION</span><span style={{fontSize:9,color:'#cbd5e1'}}>Done: <b>{healthData.completedVax}</b> | Due: <b style={{color:'#fcd34d'}}>{healthData.dueVax}</b></span></div></div></div>
