...
import { Milk, Sparkles, AlertTriangle, X, TrendingDown, HeartPulse, Activity, Plus, Calculator, LockKeyhole } from 'lucide-react';
...
interface OfficialComl {
  ...
  record?: { total_coml_per_liter: number } | null;
}
...
export default function UnifiedDashboard({ onNavigate, onOpenYieldModal, onOpenPassport, herdMasterList = [], realTimeTodayYield, realTimeReceivables = 0 }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [dashboard, comlResponse] = await Promise.all([
  ...
  return (
    <div className="cmd-dash-wrapper" style={{ height:'calc(100vh - 75px)', overflowY:'hidden', display:'flex', flexDirection:'column', boxSizing:'border-box', padding:10 }}>
      ...
      {!officialComl?.has_official && officialComl?.reminder_due && (
        <div onClick={() => onNavigate?.('coml')} style={{ marginBottom:8, padding:'8px 10px', background:'rgba(245,158,11,.08)', border:'1px solid #f59e0b', color:'#fcd34d', borderRadius:6, fontSize:10, display:'flex', alignItems:'center', gap:6, cursor:'pointer' }}>
          <AlertTriangle size={13} /> No official COML set for {officialComl.month_label}. Click to calculate and lock the monthly value.
        </div>
      )}
      <div className="cmd-content-grid" style={{ display:'grid', gridTemplateColumns:'1.2fr 0.8fr', gap:10, flex:1, minHeight:0 }}>
        <div className="cmd-col" style={{ display:'flex', flexDirection:'column', gap:10, minHeight:0 }}>
          <div className="cmd-card" style={{ flex:'1.6', display:'flex', flexDirection:'column', background:'#111827', border:'1px solid #1f2937', borderRadius:8, padding:10, minHeight:0 }}>
            ...
            <div className="stat-row" style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:6, marginBottom:8 }}>
              ...
              <div onClick={() => onNavigate?.('coml')} style={{ background:'#1e293b', padding:6, borderRadius:6, borderLeft:'2px solid #34d399', cursor:'pointer' }} title="Official monthly Cost of Milk Production per Liter"><div style={{ fontSize:8, color:'#34d399', fontWeight:800 }}>COML · {officialComl?.has_official ? officialComl.month_label : 'Current Month'}</div><div style={{ fontSize:13, fontWeight:900, color:officialComl?.has_official ? '#34d399' : '#f59e0b', marginTop:2 }}>{officialComl?.has_official ? `PKR ${officialComl.record!.total_coml_per_liter.toFixed(2)} / L` : 'Not officially set'}</div></div>
            </div>
            <div className="stat-row" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:8 }}><WideStat label={currentDateLabel} value={`${todayYield} L`} color={todayYieldColor}/><WideStat label={priorDateLabel} value={`${yesterdayLiters} L`} color="#cbd5e1" border="#64748b"/><WideStat label="Pending Cash" value={`Rs. ${realTimeReceivables.toLocaleString()}`} color="#f59e0b" border="#f59e0b" /></div>
            <div style={{ display:'grid', gridTemplateColumns:'1.05fr 0.95fr', gap:8, flex:1, minHeight:0 }}>
              <div style={panel}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}><span style={graphTitle}><Activity size={12}/> Total Farm Yield Trend</span><select value={chartDays} onChange={e=>setChartDays(Number(e.target.value))} style={selectStyle}><option value={7}>7 Days</option><option value={15}>15 Days</option><option value={30}>30 Days</option></select></div><div style={{flex:1,minHeight:65,position:'relative'}}><ResponsiveContainer width="100%" height="100%"><AreaChart data={filteredYieldTrend} margin={{top:2,right:6,left:-24,bottom:0}}><XAxis dataKey="dayIndex" hide/><YAxis allowDecimals={false} stroke="#64748b" tick={{fontSize:8}} domain={['auto','auto']}/><Tooltip contentStyle={{backgroundColor:'#0f172a',borderColor:'#334155',fontSize:'10px'}} labelFormatter={v=>`Day ${v}`}/><Area type="monotone" dataKey="yield" stroke="#38bdf8" strokeWidth={2} fillOpacity={1} fill="rgba(56,189,248,.18)" isAnimationActive={false}/></AreaChart></ResponsiveContainer></div></div>
              <div style={panel}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}><span style={{fontSize:10,fontWeight:800,color:'#f87171',display:'flex',alignItems:'center',gap:4}}><AlertTriangle size={11}/> Yield Drop Watchlist ({activeDropAlerts.length})</span><span style={{fontSize:8,color:'#94a3b8'}}>Click row</span></div><div style={{flex:1,overflowY:'auto',display:'flex',flexDirection:'column',gap:4}}>{activeDropAlerts.length===0 ? <div style={{fontSize:10,color:'#34d399',textAlign:'center',padding:12}}>✓ No active yield drop warnings</div> : activeDropAlerts.map((item:any)=><div key={item.id} onClick={()=>handleOpenDropComparison(item.animalId||'TD-004',item.title)} style={{background:'#161f30',borderLeft:item.currentLevel==='RED'?'3px solid #ef4444':'3px solid #f59e0b',padding:'4px 8px',borderRadius:4,display:'flex',justifyContent:'space-between',cursor:'pointer'}}><span style={{color:'#38bdf8',fontWeight:700}}>#{item.animalId || 'Animal'}</span><span style={{color:item.currentLevel==='RED'?'#ef4444':'#f59e0b',fontWeight:700}}>{item.dropPercent ? `${Math.round(item.dropPercent)}%` : 'Drop'}</span></div>)}</div></div>
            </div>
          </div>
          <div style={{ flex:1, display:'grid', gridTemplateColumns:'1.3fr 0.7fr', gap:10, minHeight:0 }}><div className="cmd-card" style={{display:'flex',flexDirection:'column',...cardBase}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}><span onClick={()=>onNavigate?.('animals')} style={{display:'flex',alignItems:'center',gap:6,color:'#f59e0b',fontWeight:800,fontSize:12,cursor:'pointer'}}><CowIcon size={16} color="#f59e0b"/> Total Herd</span><span style={{fontSize:10,color:'#94a3b8'}}>Total: {totalHerdCount} Head</span></div><div style={{flex:1,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,minHeight:0}}><HerdTable rows={herdCol1}/><HerdTable rows={herdCol2}/></div></div><div className="cmd-card" style={{display:'flex',flexDirection:'column',...cardBase}}><div style={{color:'#38bdf8',fontWeight:800,fontSize:12,marginBottom:8}}><Plus size={15} style={{verticalAlign:'middle',marginRight:4}}/> Data Entry</div><div style={{display:'flex',flexDirection:'column',gap:8,justifyContent:'center',flex:1}}><ActionButton onClick={()=>onOpenYieldModal ? onOpenYieldModal() : onNavigate?.('milk')} text="Enter Milk Production" icon={<Milk size={14}/>} color="#0284c7"/><ActionButton onClick={()=>onNavigate?.('finance')} text="Enter Milk Sale" icon={<span style={{fontWeight:900}}>₨</span>} color="#059669"/><ActionButton onClick={()=>onNavigate?.('coml')} text="Open Official COML" icon={<Calculator size={14}/>} color="#7c3aed"/></div></div></div>
        </div>
        <div className="cmd-col" style={{display:'flex',flexDirection:'column',gap:10,minHeight:0}}>
          <div className="cmd-card" style={{flex:0.8,...cardBase}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}><span style={{display:'flex',alignItems:'center',gap:6,color:'#34d399',fontWeight:800,fontSize:12}}><LockKeyhole size={14}/> Official COML Status</span><button onClick={()=>onNavigate?.('coml')} style={{background:'none',border:'none',color:'#38bdf8',fontSize:9,cursor:'pointer'}}>Open COML</button></div>{officialComl?.has_official ? <div><div style={{fontSize:22,fontWeight:900,color:'#34d399'}}>PKR {officialComl.record!.total_coml_per_liter.toFixed(2)} / L</div><div style={{fontSize:10,color:'#94a3b8',marginTop:4}}>Valid for {officialComl.month_label} · Official / Locked</div></div> : <div style={{fontSize:11,color:'#fcd34d',lineHeight:1.5}}><AlertTriangle size={14} style={{verticalAlign:'middle',marginRight:5}}/>No official COML set for {officialComl?.month_label || 'current month'}. Monthly reminder: {officialComl?.reminder_status || 'DUE'}.</div>}</div>
          ...
          <div className="cmd-card" style={{flex:0.85,...cardBase}}><div style={{display:'flex',alignItems:'center',gap:6,color:'#ef4444',fontWeight:800,fontSize:12,marginBottom:6,cursor:'pointer'}} onClick={()=>onNavigate?.('health')}><HeartPulse size={15}/> Health & Treatments</div><div style={{display:'flex',flexDirection:'column',gap:6,justifyContent:'center',flex:1}}><div style={healthBox}><span style={{fontWeight:800,color:'#fca5a5'}}><b style={{background:'#ef4444',color:'#fff',padding:'2px 4px',borderRadius:4,fontSize:8,marginRight:5}}>SICK</b>{healthData.sick} ANIMALS</span><span style={{fontSize:9,color:'#f87171'}}>Mastitis: {healthData.mastitis} | Temp: {healthData.highTemp}</span></div><div style={{...healthBox,background:'rgba(245,158,11,.10)',borderColor:'rgba(245,158,11,.3)'}}><span style={{fontWeight:800,color:'#fcd34d'}}>VACCINATION</span><span style={{fontSize:9,color:'#cbd5e1'}}>Done: <b>{healthData.completedVax}</b> | Due: <b style={{color:'#fcd34d'}}>{healthData.dueVax}</b></span></div></div></div>
...
function SmallStat({label,value,color='#fff'}:{label:string,value:string|number,color?:string}){return <div style={{background:'#1e293b',padding:6,borderRadius:6}}><div style={{fontSize:8,color:'#94a3b8'}}>{label}</div><div style={{fontSize:13,fontWeight:900,color,marginTop:2}}>{value}</div></div>}
function WideStat({label,value,color,border='#38bdf8'}:{label:string,value:string,color:string,border?:string}){return <div style={{background:'#1e293b',padding:'8px 12px',borderRadius:6,borderLeft:`3px solid ${border}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:11,color:'#94a3b8',fontWeight:800}}>{label}</div><div style={{fontSize:15,fontWeight:900,color}}>{value}</div></div>}
...
const cardBase:React.CSSProperties={background:'#111827',border:'1px solid #1f2937',borderRadius:8,padding:10,minHeight:0,overflow:'hidden'};
const healthBox:React.CSSProperties={display:'flex',justifyContent:'space-between',alignItems:'center',background:'rgba(239,68,68,.10)',border:'1px solid rgba(239,68,68,.3)',padding:'6px 8px',borderRadius:6};
