...
export default function UnifiedDashboard({ onNavigate, onOpenYieldModal, onOpenPassport, herdMasterList = [], dashboardRefreshVersion = 0 }: Props) {
  ...
  const handleOpenDropComparison = (animalId: string, fallbackAlert?: any) => {
  ...
  const extremesOptions = Array.from(
    { length: maximumExtremePopulation },
    (_, index) => index + 1,
  );
            ...
            activeDropAlerts.map((item:any, index:number) => {
              ...
              return (
                <div key={item.id} style={{display:'grid',gridTemplateColumns:'55px minmax(110px,.65fr) 90px minmax(240px,2fr) 90px',gap:8,padding:'9px 10px',borderTop:index===0?'none':'1px solid #1f2937',alignItems:'center',fontSize:10}}>
                  ...
                  <button type="button" onClick={() => item.animalId && handleOpenDropComparison(item.animalId, item)} style={{background:'none',border:0,padding:0,textAlign:'left',color:'#cbd5e1',cursor:item.animalId?'pointer':'default'}}>{dropLabel}{detailText ? ` — ${detailText}` : ''}</button>
    ...
    <div className="cmd-dash-wrapper" style={{ height:'calc(100vh - 60px)', overflow:'hidden', display:'flex', flexDirection:'column', boxSizing:'border-box', padding:10, minWidth:0 }}>
      ...
      <div className="cmd-content-grid" style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(260px,.8fr)', gap:10, flex:1, minHeight:0, minWidth:0, overflow:'hidden' }}>
        ...
        <div className="cmd-col" style={{display:'flex',flexDirection:'column',gap:10,minHeight:0,minWidth:0,overflow:'hidden'}}>
          <div className="cmd-card" role="button" tabIndex={0} aria-label="Open Production Extremes" onClick={()=>setExpandedMilkList('PRODUCTION_EXTREMES')} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setExpandedMilkList('PRODUCTION_EXTREMES')}}} style={{flex:'0.9 1 0',...cardBase,cursor:'pointer'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4,gap:6,minWidth:0}}>
              ...
              <select value={Math.min(extremesCount, Math.max(maximumExtremePopulation, 1))} onClick={e=>e.stopPropagation()} onChange={e=>{e.stopPropagation();setExtremesCount(Number(e.target.value))}} disabled={maximumExtremePopulation===0} style={selectStyle}>{extremesOptions.map(n=><option key={n} value={n}>{n}</option>)}</select>
            </div>
          ...
          <div style={{flex:'0.85 1 0',display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,minHeight:0,minWidth:0}}>
            <div className="cmd-card" role="button" tabIndex={0} aria-label="Open Clinical Health" onClick={()=>onNavigate?.('health')} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onNavigate?.('health')}}} style={{...cardBase,minWidth:0,cursor:'pointer'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:6,color:'#ef4444',fontWeight:800,fontSize:12,marginBottom:6}}><span style={{display:'flex',alignItems:'center',gap:6}}><HeartPulse size={15}/> Health</span><span style={{fontSize:9,color:'#fca5a5'}}>{sickAnimals.length} active</span></div>
...
const attentionRow:React.CSSProperties={display:'grid',gridTemplateColumns:'minmax(66px,.8fr) minmax(0,1.25fr) auto',alignItems:'center',gap:4,background:'#161f30',padding:'3px 4px',borderRadius:3,minWidth:0,fontSize:10};
