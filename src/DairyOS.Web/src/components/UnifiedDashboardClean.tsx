...
export default function UnifiedDashboardClean({ onNavigate, onOpenYieldModal, onOpenPassport, herdMasterList = [], realTimeTodayYield, realTimeReceivables = 0 }: Props) {
  ...
  useEffect(() => { fetchCommandDashboardData().then(setData).catch(() => setData(null)).finally(() => setLoading(false)); }, []);
  ...
  const conceptionRatio = (data?.reproduction as any)?.conceptionRatio ?? '—';
  const trend = useMemo(() => {
