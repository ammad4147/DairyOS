...
const domains: { key: DomainKey; label: string }[] = [
  ...
  { key: 'MILK', label: 'Milk' },
  { key: 'MILK_QUALITY', label: 'Milk Quality' },
  { key: 'FEED', label: 'Feed / TMR' },
  { key: 'FINANCE', label: 'Finance' },
  { key: 'BREEDING', label: 'Breeding' },
  { key: 'SEMEN', label: 'Semen Inventory' },
  { key: 'HEALTH', label: 'Health' },
  { key: 'VACCINATION', label: 'Vaccination' },
  { key: 'COML', label: 'COML / COP' },
  { key: 'WHOLE_FARM', label: 'Whole Farm' },
];
...
export default function ReportingTab() {
  ...
  const [cohort, setCohort] = useState('All eligible animals');
  ...
  const selectDomain = (next: DomainKey) => {
    setDomain(next);
    setReportId(reports.find(item => item.domain === next)?.id ?? '');
    setPreviewed(false);
  };
  ...
  const saveReport = () => {
    ...
    const lines = [
      `DairyOS ${report.name}`,
      `Domain: ${domains.find(item => item.key === domain)?.label}`,
  ...
  const showAnimal = ['ANIMALS', 'MILK', 'BREEDING', 'HEALTH', 'VACCINATION'].includes(domain);
  const showCategory = ['ANIMALS', 'MILK', 'FEED'].includes(domain);
  const showSession = domain === 'MILK';
  const showCohort = domain === 'MILK';
  const snapshot = domain === 'WHOLE_FARM';

  return (
    <div style={{ display: 'grid', gap: 12 }}>
      <section style={card}>
        <div style={{ display: 'grid', gridTemplateColumns: '190px 1fr 1fr 130px', gap: 9, alignItems: 'end' }}>
          <div>
            <label style={label}>Domain</label>
            <select value={domain} onChange={event => selectDomain(event.target.value as DomainKey)} style={field}>
              {domains.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}
            </select>
          </div>
          <div>
            <label style={label}>Report</label>
            <select value={report.id} onChange={event => { setReportId(event.target.value); setPreviewed(false); }} style={field}>
              {availableReports.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}
            </select>
          </div>
          <div>
            <label style={label}>{snapshot ? 'Snapshot Date' : 'As Of / Operational Date'}</label>
            <input type="date" value={asOfDate} onChange={event => setAsOfDate(event.target.value)} style={field} />
          </div>
          <div>
            <label style={label}>Format</label>
            <select value={format} onChange={event => setFormat(event.target.value as Format)} style={field}>
      ...
      <section style={card}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 9, alignItems: 'end' }}>
          {!snapshot && <div><label style={label}>Start Date</label><input type="date" value={startDate} onChange={event => setStartDate(event.target.value)} style={field} /></div>}
          {!snapshot && <div><label style={label}>End Date</label><input type="date" value={endDate} onChange={event => setEndDate(event.target.value)} style={field} /></div>}
          {showCategory && <div><label style={label}>Category</label><select value={category} onChange={event => setCategory(event.target.value)} style={field}>{canonicalCategories.map(item => <option key={item}>{item}</option>)}</select></div>}
          {showSession && <div><label style={label}>Session</label><select value={session} onChange={event => setSession(event.target.value)} style={field}>{sessions.map(item => <option key={item}>{item}</option>)}</select></div>}
      ...
      <section id="dairyos-report-preview" style={{ ...card, background: '#0b1220' }}>
        ...
        <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1.2fr .8fr', gap: 10 }}>
          <div style={{ borderTop: '1px solid #1f2937', paddingTop: 9 }}>
            <div style={{ ...label, marginBottom: 6 }}>Authority</div>
            <div style={{ color: '#e2e8f0', fontSize: 11, lineHeight: 1.45 }}>{report.authority}</div>
            <div style={{ ...label, margin: '12px 0 6px' }}>Scope</div>
            <div style={{ color: '#cbd5e1', fontSize: 11, lineHeight: 1.45 }}>{report.scopeNote}</div>
          </div>
          <div style={{ borderTop: '1px solid #1f2937', paddingTop: 9 }}>
            <div style={label}>Applicable Controls</div>
            <div style={{ color: '#cbd5e1', fontSize: 10, marginTop: 6 }}>Periods: {report.periods.join(', ')}</div>
            <div style={{ color: '#cbd5e1', fontSize: 10, marginTop: 5 }}>Filters: {report.filters.join(', ')}</div>
      ...
      <section style={card}>
        <div style={{ fontSize: 12, fontWeight: 900, color: '#cbd5e1', marginBottom: 8 }}>Report Catalog and Authority Matrix</div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860, fontSize: 10 }}>
            <thead><tr style={{ color: '#94a3b8', textAlign: 'left' }}><th>Domain</th><th>Reports</th><th>Authority</th><th>Filters</th><th>Periods</th><th>Output</th></tr></thead>
            <tbody>
              {domains.map(item => {
                const rows = reports.filter(reportItem => reportItem.domain === item.key);
                return <tr key={item.key} style={{ borderTop: '1px solid #1f2937', verticalAlign: 'top' }}><td style={{ padding: 8, fontWeight: 900, color: '#e2e8f0' }}>{item.label}</td><td style={{ padding: 8 }}>{rows.map(row => row.name).join(', ')}</td><td style={{ padding: 8, color: '#94a3b8' }}>{rows[0]?.authority}</td><td style={{ padding: 8 }}>{Array.from(new Set(rows.flatMap(row => row.filters))).join(', ')}</td><td style={{ padding: 8 }}>{Array.from(new Set(rows.flatMap(row => row.periods))).join(', ')}</td><td style={{ padding: 8 }}>PDF, XLSX, CSV, Print</td></tr>;
              })}
            </tbody>
          </table>
        </div>
