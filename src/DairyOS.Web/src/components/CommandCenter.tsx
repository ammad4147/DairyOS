...
function displayNumber(value: unknown, suffix = ""): string {
    ...
    return number === null ? "—" : `${number.toLocaleString(undefined, { maximumFractionDigits: 1 })}${suffix}`;
}
...
function CommandCenter({ onOpenAnimal }: Props) {
    ...
    if (loading && !dashboard) {
        return <main className="command-center"><div className="farm-loading">Loading live farm operations…</div></main>;
    }
    ...
    return (
        <main className="command-center">
            <header className="farm-header">
                <div>
                    <span className="farm-eyebrow">TRIDENT DAIRIES · INTELLIGENT FARM OPERATIONS</span>
                        ...
                        {showNotifications && (
                            <div className="notification-popover">
                                ...
                                {yieldAlerts.map((alert) => (
                                    <button key={alert.id} type="button" className="notification-row" onClick={() => onOpenAnimal?.(alert.animalId)}>
                                        <span className="notification-id">{alert.id}</span>
                                        <strong>{alert.animalId}: milk yield down {alert.dropPercent.toFixed(1)}%</strong>
                                        <small>Current {displayNumber(alert.current, " L")} · Previous {displayNumber(alert.previous, " L")}</small>
                                    </button>
                                ))}
                                {decisions.slice(0, 8).map((decision, index) => (
                                    <button key={`${decision.type ?? "decision"}-${decision.animal_id ?? index}`} type="button" className="notification-row" onClick={() => decision.animal_id && onOpenAnimal?.(decision.animal_id)}>
                                        ...
                                        <small>{decision.animal_id ?? "Farm-level"} · {String(decision.priority ?? "attention")}</small>
                                    </button>
                                ))}
            ...
            <section className="command-grid">
                ...
                <article className="dashboard-card herd-card">
                    ...
                    <button type="button" className="text-link" onClick={() => setHerdDetail("milking")}>View herd details →</button>
                </article>
                ...
                <article className="dashboard-card attention-card">
                    ...
                    <div className="attention-preview">
                        {yieldAlerts.slice(0, 3).map((alert) => <button key={alert.id} type="button" onClick={() => onOpenAnimal?.(alert.animalId)}><span>{alert.id}</span><strong>{alert.animalId} · −{alert.dropPercent.toFixed(1)}% daily yield</strong></button>)}
                        {!yieldAlerts.length && <div className="attention-ok">No animal yield drop above 20% detected.</div>}
                    </div>
                    <button type="button" className="text-link" onClick={() => setShowNotifications(true)}>Open notifications →</button>
                </article>
            </section>
                ...
                <div className="modal-backdrop" role="presentation" onMouseDown={() => setHerdDetail(null)}>
                    <section className="herd-modal" role="dialog" aria-modal="true" aria-label="Herd details" onMouseDown={(event) => event.stopPropagation()}>
                        ...
                        <div className="animal-detail-list">
                            {activeHerdRows.length ? activeHerdRows.map((row) => <button key={row.id} type="button" onClick={() => onOpenAnimal?.(row.id)}><strong>{row.id}</strong><span>{row.lifecycle}</span></button>) : <div className="notification-empty">No animals in this category.</div>}
                        </div>
                    </section>
                </div>
            )}
