...
export default function DigitalTwinPanel(){
 ...
 const scenarioRisk=twin?.scenario_summary?.risk_level;
 return <section style={panel}>
  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8,flexWrap:'wrap',marginBottom:9}}><div><div style={{fontSize:13,fontWeight:800,display:'flex',alignItems:'center',gap:6}}><BrainCircuit size={14} color="#a78bfa"/> Digital Twin â€” Scenario Lab</div><div style={{fontSize:9,color:'#64748b'}}>Forecasts and what-if scenarios over persisted farm baselines. Assumptions are explicit; forecasts never become farm facts.</div></div></div>
  ...
  {baseline&&<div style={{marginTop:8,display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:7}}><div style={metricCard}><div style={eyebrow}>Live baseline</div><div style={value}>{Number(baseline.baseline_value).toLocaleString()}</div><div style={muted}>{baseline.metric} Â· {baseline.baseline_period_days} days</div></div><div style={metricCard}><div style={eyebrow}>Forecast</div><div style={value}>{forecast==null?'â€”':Number(forecast).toLocaleString()}</div><div style={muted}>{result?.horizon_days||horizonDays} day horizon</div></div><div style={metricCard}><div style={eyebrow}>Scenario risk</div><div style={{...value,color:'#fbbf24'}}>{scenarioRisk||'â€”'}</div><div style={muted}>{result?.scenario_name||scenarioName}</div></div></div>}
