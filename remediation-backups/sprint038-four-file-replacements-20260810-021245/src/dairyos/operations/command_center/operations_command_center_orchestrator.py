...
class OperationsCommandCenterOrchestrator:
    ...
    def __init__(
        ...
        self.health_service = (
            self.runtime.operations_health_service
        )

        self.command_center_service = (
            self.runtime.operations_command_service
        )
    ...
    def _build_canonical_command_center_projection(self):
        ...
        if hasattr(command_center, "__dict__"):
            view = command_center.__dict__.copy()
        else:
            view = dict(command_center)

        farm_status = view.get(
            "farm_status",
            {},
        )

        if hasattr(farm_status, "__dict__"):
            farm_status = farm_status.__dict__.copy()

        if not isinstance(farm_status, dict):
            farm_status = {}

        view["farm_status"] = farm_status
    ...
    def generate_command_center(self):
        ...
        view = (
            self._build_canonical_command_center_projection()
        )
