...
function Invoke-Step {
    ...
    try {
        # Native tools such as npm may write warnings to stderr while still
        # returning exit code 0. Do not let PowerShell promote those warnings
...
try {
    ...
    $results.Add([pscustomobject]@{
        ...
        Passed = (Invoke-Step "Python compileall" {
    ...
    $results.Add([pscustomobject]@{
        ...
        Passed = (Invoke-Step "Full pytest regression" {
            pytest -q
        })
    })
    ...
    if (-not $SkipDatabaseBackup) {
        ...
        if (Test-Path $backupScript) {
            ...
            $results.Add([pscustomobject]@{
                ...
                Passed = (Invoke-Step "Database backup" {
                    python $backupScript backup --output $dump
                })
            })

            $results.Add([pscustomobject]@{
                Name   = "Database backup verification"
                Passed = (Invoke-Step "Database backup verification" {
                    python $backupScript verify --input $dump
                })
            })
        }
        else {
        ...
        $results.Add([pscustomobject]@{
            ...
            Passed = (Invoke-Step "OS handover Phase 0 audit" {
                ...
                if ($LASTEXITCODE -eq 2) {
                    throw "OS handover gate is BLOCKED."
                }
            })
        })
    }
...
Write-Host "Completed stages: $($results.Count); failed/blocked: $($failed.Count)" `
